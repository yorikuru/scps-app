...
export type EmailCategory = 
  | "default" | "auth" | "notice" | "action" | "admin_support" 
  | "alerts" | "noreply" | "billing" | "news" | "export";
...
export async function sendEmailMessage(
  ...
  if (!to) return { success: false, error: "宛先が指定されていません。" };
  ...
  try {
    ...
    if (!tokenResponse.ok) return { success: false, error: "メールサーバーの認証に失敗しました。" };
    ...
    const mailContent: any = {
      message: {
        subject: subject,
        from: { emailAddress: { name: sender.name, address: sender.address } },
    ...
    // ★ 添付ファイルが存在する場合はAPIの形式にマッピングして追加
    if (attachments && attachments.length > 0) {
      mailContent.message.attachments = attachments.map(att => ({
    ...
    if (!mailResponse.ok) return { success: false, error: "メールの送信に失敗しました。" };
...
export async function sendNotificationEmailToUser(
  ...
  try {
    ...
    if (!userDoc.exists) return { success: false, error: "ユーザーが存在しません。" };

    const userData = userDoc.data();
    const email = userData?.email;
    const isNotificationEnabled = userData?.emailNotificationEnabled !== false;

    if (!email) return { success: false, error: "このユーザーにはメールアドレスが登録されていません。" };
    if (!isImportant && !isNotificationEnabled) return { success: true, message: "ユーザーがメール通知をOFFにしているため配信をスキップしました。" };
