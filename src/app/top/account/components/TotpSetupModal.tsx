...
type Props = {
  ...
  onClose: () => void;
  // ★古い showAlert プロパティ定義は削除しました
};

export default function TotpSetupModal({ currentUser, userData, setUserData, onClose }: Props) {
  ...
  const handleStartTotpSetup = async () => {
    if (!currentUser?.email) {
      showAlert("メールアドレスが登録されていないため、設定できません。", "error");
      onClose();
      return;
    }
    ...
    try {
      ...
      showAlert("設定用データの生成に失敗しました。再度お試しください。", "error");
      onClose();
    }
  };
  ...
  const handleVerifyTotp = async () => {
    ...
    try {
      ...
      setSetupStep(4); 
      showAlert("認証アプリの設定が完了しました！", "success");
