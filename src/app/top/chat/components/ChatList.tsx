...
export default function ChatList({ 
  ...
  const renderRoomItem = (room: ChatRoom, isPinned: boolean) => {
    ...
    return (
      ...
      >
        {info.isGroup ? (
          <div className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center text-white flex-shrink-0 shadow-sm relative ${room.iconURL ? 'bg-transparent border border-gray-200' : roomConfig?.color || 'bg-gray-700'}`}>
            {room.iconURL ? (
              ...
              <roomConfig.icon className="w-3.5 h-3.5 sm:w-4 sm:h-4"/>
            ) : (
              <Users className="w-3.5 h-3.5 sm:w-4 sm:h-4"/>
            )}
          </div>
        ) : (
          <UserAvatar name={info.name} url={info.avatar} isExternal={info.isExternal} className="w-8 h-8 sm:w-9 sm:h-9 text-xs" presenceState={presenceState} />
        )}
        <div className="flex-1 min-w-0 pr-6">
          <div className="flex justify-between items-end mb-0.5">
            <div className="flex items-center gap-1 overflow-hidden">
              {room.isOfficial && <span className="px-1 py-0.5 rounded text-[6px] sm:text-[7px] font-black bg-indigo-100 text-indigo-700 flex-shrink-0 border border-indigo-200">公式</span>}
              {info.isExternal && <span className="px-1 py-0.5 rounded text-[6px] sm:text-[7px] font-black bg-amber-100 text-amber-700 flex-shrink-0 border border-amber-200">外部</span>}
              <h4 className={`text-[11px] sm:text-[12px] truncate ${isSelected ? c.text : 'text-gray-900'} ${unreadCount > 0 ? 'font-black' : 'font-bold'}`}>{info.name}</h4>
            </div>
            <span className={`text-[7px] sm:text-[8px] font-bold flex-shrink-0 ml-1 ${unreadCount > 0 ? 'text-red-500' : 'text-gray-400'}`}>{formatTime(room.updatedAt)}</span>
          </div>
          <div className="flex justify-between items-center gap-2">
            <p className={`text-[9px] sm:text-[10px] truncate ${unreadCount > 0 || isMessageHit ? 'text-gray-900 font-bold' : 'text-gray-500 font-medium'}`}>
              {isMessageHit && <Search className="w-2.5 h-2.5 inline-block mr-0.5 text-indigo-500" />}
              {previewMessage}
            </p>
            <div className="flex items-center gap-1 shrink-0">
              {isMentioned && (
                <span className="bg-rose-100 text-rose-700 border border-rose-200 text-[6px] sm:text-[7px] font-black px-1 py-0.5 rounded">
                  @メンション
                </span>
              )}
              {unreadCount > 0 && (
                <span className="bg-red-500 text-white text-[7px] sm:text-[8px] font-black px-1.5 py-0.5 rounded-full min-w-[14px] sm:min-w-[16px] text-center shadow-sm">
    ...
    <div className="flex flex-col h-full bg-white relative border-r border-gray-200">
      ...
      {showRenameModal && (
        <div className="absolute inset-0 z-50 bg-black/40 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-xs sm:max-w-sm overflow-hidden animate-slide-up">
            <div className="p-2.5 sm:p-3 border-b border-gray-100 flex justify-between items-center bg-gray-50">
              <h3 className="text-[10px] sm:text-[11px] font-black text-gray-900">グループ名の変更</h3>
            ...
            <div className="p-3 sm:p-4">
              <input 
                ...
                className="w-full px-3 py-1.5 sm:py-2 border border-gray-300 rounded-lg sm:rounded-xl text-[11px] sm:text-xs font-bold focus:ring-2 focus:ring-indigo-500 outline-none shadow-sm"
                placeholder="新しいグループ名"
                autoFocus
              />
              <div className="mt-3 sm:mt-4 flex justify-end gap-2">
                <button onClick={() => setShowRenameModal(null)} className="px-3 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-bold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg sm:rounded-xl transition-colors shadow-2xs">キャンセル</button>
                <button onClick={handleRenameRoom} disabled={!newName.trim()} className="px-3 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-bold text-white bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-300 rounded-lg sm:rounded-xl transition-colors shadow-sm">保存する</button>
              </div>
            </div>
          </div>
        </div>
      ...
      <div className="p-2 sm:p-3 border-b border-gray-100 bg-gray-50/50 shrink-0">
        <div className="flex justify-between items-center mb-2">
          <h2 className="text-[11px] sm:text-xs font-black text-gray-900 px-1">トークを選択</h2>
        ...
        {!isExternalMode && (
          <div className="flex bg-gray-200/60 p-0.5 sm:p-1 rounded-lg w-full shadow-inner mb-2 overflow-x-auto no-scrollbar">
            <button onClick={() => setActiveTab("chats")} className={`relative flex-1 min-w-[45px] flex justify-center items-center py-1 sm:py-1.5 text-[8px] sm:text-[9px] font-bold rounded transition-all ${activeTab === "chats" ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700"}`}>
              トーク
              {totalUnreadCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-500 text-white text-[6px] sm:text-[7px] font-black px-1 rounded-full shadow-sm z-10 border border-white">
                  {totalUnreadCount > 99 ? '99+' : totalUnreadCount}
                </span>
              )}
            </button>
            <button onClick={() => setActiveTab("contacts")} className={`flex-1 min-w-[50px] flex justify-center items-center py-1 sm:py-1.5 text-[8px] sm:text-[9px] font-bold rounded transition-all ${activeTab === "contacts" ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700"}`}>アドレス</button>
            
            {isInternalUser && perms.canViewExternalUser && (
              <button onClick={() => setActiveTab("external")} className={`flex-1 min-w-[50px] flex justify-center items-center py-1 sm:py-1.5 text-[8px] sm:text-[9px] font-bold rounded transition-all ${activeTab === "external" ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700"}`}>外部管理</button>
            )}
            
            {isInternalUser && internalUser && (internalUser.role === "admin" || internalUser.role === "system_admin" || internalUser.isITManager) && (
              <button onClick={() => setActiveTab("settings")} className={`flex-1 min-w-[45px] flex justify-center items-center py-1 sm:py-1.5 text-[8px] sm:text-[9px] font-bold rounded transition-all ${activeTab === "settings" ? "bg-white shadow-sm text-gray-900" : "text-gray-500 hover:text-gray-700"}`}>設定</button>
            )}
          </div>
        )}
        ...
        <div className="relative">
          ...
          <input type="text" placeholder="検索..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className={`w-full pl-7 pr-2 py-1.5 sm:py-1.5 bg-white border border-gray-200 rounded-lg text-[10px] sm:text-[11px] font-bold focus:outline-none focus:ring-2 ${c.ring} shadow-2xs placeholder:text-gray-400`} />
        </div>
      </div>
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar relative">
          ...
          <div className="pb-4">
            {filteredRooms.length === 0 ? (
              <div className="py-10 text-center text-gray-400 flex flex-col items-center">
                <MessageCircle className="w-5 h-5 sm:w-6 sm:h-6 mb-1.5 sm:mb-2 opacity-30" />
                <p className="text-[9px] sm:text-[10px] font-bold">トーク履歴がありません</p>
              </div>
            ) : (
              <>
                {pinnedRooms.length > 0 && (
                  <div className="mb-1.5 sm:mb-2">
                    <div className="px-2.5 py-1 bg-gray-50 border-y border-gray-100 text-[8px] sm:text-[9px] font-bold text-gray-500 flex items-center gap-1"><Pin className="w-2.5 h-2.5" /> ピン留め</div>
                    <div className="divide-y divide-gray-50">{pinnedRooms.map(r => renderRoomItem(r, true))}</div>
                  </div>
                )}
                {otherRooms.length > 0 && (
                  <div>
                    {pinnedRooms.length > 0 && <div className="px-2.5 py-1 bg-gray-50 border-y border-gray-100 text-[8px] sm:text-[9px] font-bold text-gray-500">すべてのトーク</div>}
        ...
        {!isExternalMode && activeTab === "contacts" && (
          <div className="p-2 sm:p-3 pb-6">
            {!searchQuery && isInternalUser && internalUser && (
              <>
                <h5 className="text-[8px] sm:text-[9px] font-black text-gray-400 uppercase tracking-wider mb-1 sm:mb-1.5 pl-1">あなたのアカウント</h5>
                <div 
                  onClick={() => {
                    const evt = new CustomEvent('open-profile', { detail: userData });
                    window.dispatchEvent(evt);
                  }} 
                  className="flex items-center gap-2 sm:gap-2.5 p-1.5 sm:p-2 mb-2 sm:mb-3 bg-gray-50 rounded-xl border border-gray-100 cursor-pointer hover:bg-gray-100 transition-colors"
                >
                  <UserAvatar name={userData.name} url={(userData as any).photoURL} className="w-8 h-8 sm:w-10 sm:h-10 text-[10px] sm:text-sm" presenceState={getUserPresenceState(userData.id)} />
                  <div className="flex-1 min-w-0">
                    <h4 className="text-[11px] sm:text-xs font-black text-gray-900 truncate">{userData.name}</h4>
                    <span className="text-[8px] sm:text-[9px] font-bold text-gray-500 truncate block">{internalUser.positionName || "自分のプロフィール"}</span>
                  </div>
                  <ChevronRight className="w-3.5 h-3.5 text-gray-300" />
                </div>

                <h5 className="text-[8px] sm:text-[9px] font-black text-gray-400 uppercase tracking-wider mb-1 sm:mb-1.5 pl-1">所属チーム・組織</h5>
                <div className="space-y-0.5 mb-3 sm:mb-4">
                  
                  <div onClick={() => onJoinOfficialRoom({ type: "tenant_all", name: `全メンバーチャット (${schoolName || "テナント"})` })} className="flex items-center gap-2 sm:gap-2.5 p-1.5 hover:bg-blue-50 rounded-lg cursor-pointer transition-colors group border border-transparent hover:border-blue-100">
                    <div className={`w-7 h-7 sm:w-8 sm:h-8 rounded-full flex items-center justify-center text-white shadow-2xs ${schoolLogoURL ? 'bg-white border border-gray-200' : 'bg-blue-600'}`}>
                      {schoolLogoURL ? <img src={schoolLogoURL} className="w-full h-full rounded-full object-cover" alt="logo" /> : <Building2 className="w-3 h-3 sm:w-3.5 sm:h-3.5"/>}
                    </div>
                    <div className="flex-1 min-w-0">
                      <h4 className="text-[10px] sm:text-[11px] font-black text-gray-900 group-hover:text-blue-800 truncate">全メンバーチャット ({schoolName || "テナント"})</h4>
                      <span className="text-[7px] sm:text-[8px] font-bold text-gray-500">組織全員へのお知らせ等に</span>
                    </div>
                  ...
                  {(internalUser.role === "admin" || internalUser.role === "system_admin" || internalUser.isITManager) && (
                    <div onClick={() => onJoinOfficialRoom({ type: "role_admin", name: "管理者・IT担当チーム" })} className="flex items-center gap-2 sm:gap-2.5 p-1.5 hover:bg-slate-50 rounded-lg cursor-pointer transition-colors group border border-transparent hover:border-slate-200">
                      <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-slate-800 flex items-center justify-center text-white shadow-2xs"><Shield className="w-3 h-3 sm:w-3.5 sm:h-3.5"/></div>
                      <div className="flex-1 min-w-0">
                        <h4 className="text-[10px] sm:text-[11px] font-black text-gray-900 group-hover:text-slate-800">管理者・IT担当チーム</h4>
                        <span className="text-[7px] sm:text-[8px] font-bold text-gray-500">システム管理・運用連絡</span>
                      </div>
                  ...
                  {internalUser.role === "teacher" && (
                    <div onClick={() => onJoinOfficialRoom({ type: "role_teacher", name: "教職員チーム" })} className="flex items-center gap-2 sm:gap-2.5 p-1.5 hover:bg-emerald-50 rounded-lg cursor-pointer transition-colors group border border-transparent hover:border-emerald-100">
                      <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-emerald-600 flex items-center justify-center text-white shadow-2xs"><GraduationCap className="w-3 h-3 sm:w-3.5 sm:h-3.5"/></div>
                      <div className="flex-1 min-w-0">
                        <h4 className="text-[10px] sm:text-[11px] font-black text-gray-900 group-hover:text-emerald-800">教職員チーム</h4>
                        <span className="text-[7px] sm:text-[8px] font-bold text-gray-500">先生間での連絡用</span>
                      </div>
                  ...
                  {internalUser.isManager && (
                    <div onClick={() => onJoinOfficialRoom({ type: "role_manager", name: "マネージャーチーム" })} className="flex items-center gap-2 sm:gap-2.5 p-1.5 hover:bg-rose-50 rounded-lg cursor-pointer transition-colors group border border-transparent hover:border-rose-100">
                      <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-rose-600 flex items-center justify-center text-white shadow-2xs"><Star className="w-3 h-3 sm:w-3.5 sm:h-3.5"/></div>
                      <div className="flex-1 min-w-0">
                        <h4 className="text-[10px] sm:text-[11px] font-black text-gray-900 group-hover:text-rose-800">マネージャーチーム</h4>
                        <span className="text-[7px] sm:text-[8px] font-bold text-gray-500">リーダー陣の連絡用</span>
                      </div>
                  ...
                  {myPositions.map(pos => (
                    <div key={pos.id} onClick={() => onJoinOfficialRoom({ type: "position", targetId: pos.id, name: pos.name })} className="flex items-center gap-2 sm:gap-2.5 p-1.5 hover:bg-emerald-50 rounded-lg cursor-pointer transition-colors group border border-transparent hover:border-emerald-100">
                      <div className="w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-emerald-600 flex items-center justify-center text-white shadow-2xs"><Briefcase className="w-3 h-3 sm:w-3.5 sm:h-3.5"/></div>
                      <div className="flex-1 min-w-0">
                        <h4 className="text-[10px] sm:text-[11px] font-black text-gray-900 group-hover:text-emerald-800">{pos.name}</h4>
                        <span className="text-[7px] sm:text-[8px] font-bold text-gray-500">役職メンバー公式チャット</span>
                      </div>
                      <ChevronRight className="w-3.5 h-3.5 text-gray-300 group-hover:text-emerald-400" />
                    </div>
                  ))}
            ...
            <h5 className="text-[8px] sm:text-[9px] font-black text-gray-400 uppercase tracking-wider mb-1 sm:mb-1.5 pl-1">すべてのメンバー ({allFilteredUsers.length})</h5>
                ...
                return sorted.map(u => {
                  ...
                  return (
                    <div 
                      ...
                      className="flex items-center gap-2 sm:gap-2.5 p-1.5 hover:bg-gray-50 rounded-lg cursor-pointer transition-colors group"
                    >
                      <UserAvatar name={u.name} url={(u as any).photoURL} isExternal={isExt} className="w-7 h-7 sm:w-8 sm:h-8 text-[10px] sm:text-xs" presenceState={presenceState} />
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-1 sm:gap-1.5">
                          <h4 className="text-[10px] sm:text-[11px] font-black text-gray-900 truncate">{u.name}</h4>
                          {isExt && <span className="px-1 py-0.5 rounded text-[6px] sm:text-[7px] font-black bg-amber-100 text-amber-700 border border-amber-200">外部</span>}
                        </div>
                        <p className="text-[7px] sm:text-[8px] font-bold text-gray-500 truncate block">
        ...
        {!isExternalMode && activeTab === "external" && perms.canViewExternalUser && (
          <div className="p-2 sm:p-3 pb-6 animate-fade-in">
            <div className="flex justify-between items-center mb-2">
              <div>
                <h3 className="text-[10px] sm:text-[11px] font-black text-gray-900 flex items-center gap-1 sm:gap-1.5"><Globe className="w-2.5 h-2.5 sm:w-3 sm:h-3 text-amber-500"/> 外部ユーザー</h3>
                <p className="text-[7px] sm:text-[8px] font-bold text-gray-500 mt-0.5">({filteredExternalUsers.length}名)</p>
              </div>
              {perms.canCreateExternalUser && (
                <button onClick={() => onOpenExternalUserManagement && onOpenExternalUserManagement("create")} className="px-2 py-1 bg-amber-500 text-white text-[8px] sm:text-[9px] font-bold rounded shadow-sm hover:bg-amber-600 transition-colors">新規登録</button>
              )}
            </div>

            {/* ★ CustomSelect によるフィルター（折り返し対応） */}
            <div className="flex flex-wrap gap-1.5 mb-2 bg-gray-50 p-1.5 rounded-lg border border-gray-100">
              <div className="flex-1 min-w-[120px] flex items-center gap-1 bg-white border border-gray-200 rounded px-1.5 py-0.5 shadow-2xs">
                <Filter className="w-2.5 h-2.5 text-gray-400 flex-shrink-0" />
                <CustomSelect 
                  value={extCategoryFilter} 
                  onChange={setExtCategoryFilter} 
                  options={[
                  ...
                  buttonClassName="w-full text-[9px] font-bold bg-transparent border-none p-0 focus:ring-0 cursor-pointer text-gray-700 flex justify-between items-center"
                />
              </div>
              <div className="flex-1 min-w-[120px] flex items-center gap-1 bg-white border border-gray-200 rounded px-1.5 py-0.5 shadow-2xs">
                <ArrowUpDown className="w-2.5 h-2.5 text-gray-400 flex-shrink-0" />
                <CustomSelect 
                  value={extSortOrder} 
                  onChange={(val) => setExtSortOrder(val as any)} 
                  options={[
                    { value: "created", label: "登録日順" },
                    { value: "name", label: "名前順" },
                    { value: "validUntil", label: "有効期限順" }
                  ]}
                  buttonClassName="w-full text-[9px] font-bold bg-transparent border-none p-0 focus:ring-0 cursor-pointer text-gray-700 flex justify-between items-center"
                />
              </div>
            ...
            <div className="space-y-1">
              {filteredExternalUsers.map(ext => (
                <div key={ext.id} onClick={() => onOpenExternalUserManagement && onOpenExternalUserManagement("view", ext)} className="p-1.5 sm:p-2 bg-white border border-gray-200 hover:border-amber-400 rounded-lg flex items-center justify-between cursor-pointer transition-all shadow-2xs group">
                  <div className="flex items-center gap-2 min-w-0">
                    <UserAvatar name={ext.name} isExternal={true} className="w-6 h-6 sm:w-7 sm:h-7 text-[9px] sm:text-[10px]" />
                    <div className="min-w-0">
                      <div className="flex items-center gap-1">
                        <h4 className="text-[10px] sm:text-[11px] font-black text-gray-900 truncate">{ext.name}</h4>
                        <span className={`px-1 py-0.5 rounded text-[6px] sm:text-[7px] font-black ${ext.status === 'active' ? 'bg-emerald-100 text-emerald-700' : (ext.status === 'verifying' || ext.status === 'verified') ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500'}`}>
                          {ext.status === "pending" ? "仮登録" : ext.status === "verifying" ? "メール確認中" : ext.status === "verified" ? "PW設定待ち" : ext.status === "active" ? "有効" : "停止"}
                        </span>
                      </div>
                      <p className="text-[7px] sm:text-[8px] text-gray-500 truncate">{ext.affiliation || "所属なし"}</p>
                    </div>
                  </div>
                  <ChevronRight className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-gray-300 group-hover:text-amber-500 transition-colors shrink-0" />
                </div>
              ))}
            </div>
          </div>
        ...
        {!isExternalMode && activeTab === "settings" && onOpenSettings && (
          <div className="p-2 sm:p-3 pb-6 animate-fade-in">
            <div className="flex justify-between items-center mb-2 sm:mb-3">
              <div>
                <h3 className="text-[10px] sm:text-[11px] font-black text-gray-900 flex items-center gap-1 sm:gap-1.5"><Settings className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-indigo-500"/> チャット権限管理</h3>
                <p className="text-[7px] sm:text-[8px] font-bold text-gray-500 mt-0.5">カテゴリ別の機能・送信制限</p>
              </div>
            </div>

            <div className="space-y-1.5">
              <div onClick={() => onOpenSettings("general")} className="p-2 sm:p-2.5 bg-white border border-gray-200 hover:border-indigo-400 rounded-lg sm:rounded-xl flex items-center justify-between cursor-pointer transition-all shadow-2xs group">
                <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                  <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-md bg-indigo-50 flex items-center justify-center text-indigo-600 shrink-0">
                    <MessageCircle className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                  </div>
                  <div className="min-w-0">
                    <h4 className="text-[10px] sm:text-[11px] font-black text-gray-900 truncate">1. チャット全般設定</h4>
                    <p className="text-[7px] sm:text-[8px] text-gray-500 truncate">チャット機能・自由グループ作成</p>
                  </div>
                </div>
                <ChevronRight className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-gray-300 group-hover:text-indigo-500 transition-colors shrink-0" />
              </div>

              <div onClick={() => onOpenSettings("media")} className="p-2 sm:p-2.5 bg-white border border-gray-200 hover:border-indigo-400 rounded-lg sm:rounded-xl flex items-center justify-between cursor-pointer transition-all shadow-2xs group">
                <div className="flex items-center gap-1.5 sm:gap-2 min-w-0">
                  <div className="w-6 h-6 sm:w-7 sm:h-7 rounded-md bg-emerald-50 flex items-center justify-center text-emerald-600 shrink-0">
                    <Paperclip className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
                  </div>
                  <div className="min-w-0">
                    <h4 className="text-[10px] sm:text-[11px] font-black text-gray-900 truncate">2. メディア・ファイル権限</h4>
                    <p className="text-[7px] sm:text-[8px] text-gray-500 truncate">画像・各種ファイルの送信許可</p>
                  </div>
                </div>
                <ChevronRight className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-gray-300 group-hover:text-emerald-500 transition-colors shrink-0" />
              </div>
            </div>
          </div>
        )}
