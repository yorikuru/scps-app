...
// ★ Paperclip と Download アイコンを追加インポート
...
const CATEGORIES: Record<string, { label: string; badgeBg: string; color: string; bgColor: string; icon: React.ReactNode }> = {
  ...
  warning: { label: "重要", badgeBg: "bg-red-100 text-red-700", color: "text-red-700 border-red-200", bgColor: "bg-red-50", icon: <AlertTriangle className="w-3 h-3" /> },
  ...
  event: { label: "イベント", badgeBg: "bg-green-100 text-green-700", color: "text-green-700 border-green-200", bgColor: "bg-green-50", icon: <Calendar className="w-3 h-3" /> },
...
function InfoIcon() { return <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>; }
function WrenchIcon() { return <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>; }
function RefreshIcon() { return <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" /></svg>; }
function ClipboardIcon() { return <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01" /></svg>; }
function FileIcon() { return <svg className="w-3 h-3" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" /></svg>; }
    ...
    <>
        ...
        <div className="px-3.5 py-2.5 border-b border-gray-100 flex justify-between items-center bg-gray-50/50">
          ...
          {canManageMessages && (
            <Link href="/top/admin/messages" className="px-2 py-1 bg-blue-50 hover:bg-blue-100 text-blue-700 rounded-lg text-[9px] font-bold transition-colors flex items-center gap-1 border border-blue-100">
              <Edit2 className="w-2.5 h-2.5" /> 配信管理
            </Link>
          )}
        </div>
              ...
              return (
                <div key={msg.id} onClick={() => setSelectedMessage(msg)} className="p-2 sm:p-2.5 hover:bg-blue-50/50 cursor-pointer transition-colors flex items-center justify-between group min-w-0">
                  <div className="flex items-center gap-1.5 overflow-hidden mr-2 min-w-0">
                    <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold border flex-shrink-0 flex items-center gap-0.5 ${catInfo.bgColor} ${catInfo.color}`}>
                      {catInfo.label}
                    </span>
                    
                    {msg.isImportant && <span className="px-1.5 py-0.5 rounded text-[8px] font-bold bg-red-100 text-red-700 flex-shrink-0 flex items-center"><Pin className="w-2 h-2 mr-0.5" />緊急</span>}
                    {msg.subBadge === "update1" && <span className="px-1.5 py-0.5 rounded text-[8px] font-bold bg-yellow-100 text-yellow-800 border border-yellow-200 flex-shrink-0">更新①</span>}
                    {msg.subBadge === "update2" && <span className="px-1.5 py-0.5 rounded text-[8px] font-bold bg-orange-100 text-orange-800 border border-orange-200 flex-shrink-0">更新②</span>}
                    {/* ★ リスト画面でも添付があるかわかるようにクリップアイコンを表示 */}
                    {msg.attachments && msg.attachments.length > 0 && (
                      <Paperclip className="w-3 h-3 text-gray-400 flex-shrink-0" />
                    )}
                    
                    <p className="text-[10px] sm:text-[11px] font-bold text-gray-900 truncate group-hover:text-indigo-600 transition-colors ml-0.5">{msg.title}</p>
                  </div>
                  
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    {msg.requireResponse && (
                      isCompleted ? (
                        <span className="px-1.5 py-0.5 rounded text-[8px] font-black bg-blue-100 text-blue-800 border border-blue-200 shadow-sm flex items-center"><CheckSquare className="w-2 h-2 mr-0.5"/>完了</span>
                      ) : (
                        <span className="px-1.5 py-0.5 rounded text-[8px] font-bold bg-red-50 text-red-700 border border-red-100">未対応あり</span>
                      )
                    )}
                    <span className="text-[8px] sm:text-[9px] font-bold text-gray-400 ml-1">
                      {msg.startAt ? msg.startAt.split("T")[0].replace(/-/g, '/') : new Date(msg.createdAt).toLocaleDateString('ja-JP')}
                    </span>
                  </div>
                </div>
              );
      ...
      {selectedMessage && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-3 sm:p-4 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col border border-gray-200">
            <div className="p-3.5 border-b border-gray-100 flex justify-between items-center bg-gray-50/80">
              <div className="flex items-center gap-2">
                <span className={`px-2.5 py-0.5 rounded-lg text-[10px] font-bold border flex items-center gap-1 ${CATEGORIES[selectedMessage.category || "info"]?.bgColor || "bg-blue-50"} ${CATEGORIES[selectedMessage.category || "info"]?.color || "text-blue-700 border-blue-200"}`}>
                ...
                {selectedMessage.isImportant && <span className="px-2 py-0.5 rounded text-[9px] font-bold bg-red-600 text-white flex items-center shadow-xs"><AlertTriangle className="w-2.5 h-2.5 mr-0.5" />緊急</span>}
              </div>
              <div className="flex items-center gap-1.5">
            ...
            <div className="p-4 sm:p-6 space-y-3 max-h-[60vh] overflow-y-auto custom-scrollbar">
              <h3 className="text-base sm:text-lg font-black text-gray-900 leading-snug">{selectedMessage.title}</h3>
              <div className="text-[10px] text-gray-500 flex flex-col gap-1 font-bold border-b border-gray-100 pb-3">
              ...
              <div className="p-3.5 bg-gray-50 rounded-xl text-xs sm:text-sm text-gray-700 whitespace-pre-wrap leading-relaxed border border-gray-100 min-h-[90px]">
                {selectedMessage.content}
              </div>

              {/* ★ ここから：添付ファイルがある場合の表示UI */}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    {selectedMessage.attachments.map((file, idx) => (
                      <a 
                        ...
                        className="flex items-center justify-between p-2.5 bg-white border border-gray-200 rounded-xl hover:border-blue-300 hover:bg-blue-50/50 transition-all group shadow-sm"
                      >
                        <div className="flex items-center gap-2 overflow-hidden flex-1">
                          <div className="p-1.5 bg-blue-100 rounded-lg">
                            <FileIcon />
                          </div>
                          <span className="text-xs font-bold text-gray-700 group-hover:text-blue-700 truncate">{file.name}</span>
                        </div>
                ...
                <div className="mt-4 bg-blue-50 border border-blue-100 rounded-xl p-3 animate-fade-in">
                  <div className="flex justify-between items-center mb-2 border-b border-blue-100 pb-2">
                    <span className="text-xs font-bold text-blue-900 flex items-center">
                      ...
                      テナント内の対応状況 ({selectedMessage.responseType === 'single' ? "誰か1人が対応で完了" : "対象者全員の対応が必須"})
                    </span>
                  </div>
                  <div className="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto custom-scrollbar mt-2">
                    {(() => {
                      ...
                      if (selectedMessage.responseType === "single") {
                        if (isComp) return <span className="text-[10px] font-bold text-blue-700 bg-white px-2 py-1 rounded border border-blue-200 shadow-sm">対応完了: {respondedUsers.map(u=>u.name).join(", ")}</span>;
                        return <span className="text-[10px] font-bold text-gray-500">対象者のうち誰か1人が対応すると完了になります</span>;
                      } else {
                        if (isComp && uList.length > 0) return <span className="text-[10px] font-bold text-green-600">テナント全員対応完了しています</span>;
                        if (unresponded.length === 0) return <span className="text-[10px] font-bold text-gray-400">対象者がいません</span>;
                        return unresponded.map(u => <span key={u.id} className="inline-flex text-[10px] font-bold bg-white text-gray-700 px-2 py-1 rounded border border-gray-200 shadow-sm">{u.name}</span>);
                      }
                    })()}
                  </div>
                </div>
            ...
            <div className="p-3.5 border-t border-gray-100 bg-white flex justify-between items-center flex-wrap gap-3 rounded-b-2xl">
              <div className="flex gap-2">
                {selectedMessage.isDismissible && (
                  <button onClick={() => handleDeleteMessage(selectedMessage.id)} className="px-3 py-1.5 text-[11px] font-bold text-red-600 border border-red-200 hover:bg-red-50 rounded-lg transition-colors flex items-center">
              ...
              <div className="flex gap-2 ml-auto">
                {canManageMessages && !(selectedMessage.senderRole === "system_admin" || selectedMessage.schoolId === "SYSTEM") && (
                  <Link href={`/top/admin/messages?tab=form&editId=${selectedMessage.id}`} className="px-4 py-2 bg-indigo-50 text-indigo-700 hover:bg-indigo-100 rounded-xl text-xs font-bold flex items-center transition-colors border border-indigo-100">
                    <Edit2 className="w-3 h-3 mr-1" /> 管理画面で編集
                  </Link>
                )}
                <button onClick={() => setSelectedMessage(null)} className="px-5 py-2 bg-gray-900 hover:bg-black text-white text-[11px] font-bold rounded-lg shadow-sm transition-colors ml-auto">
                  閉じる
                </button>
              </div>
            </div>
