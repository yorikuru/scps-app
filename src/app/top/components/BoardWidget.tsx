    ...
    <>
        ...
        <div className="divide-y divide-gray-100 max-h-[280px] overflow-y-auto custom-scrollbar">
          {activeAnnouncements.length === 0 ? (
            ...
            activeAnnouncements.slice(0, 5).map(notice => {
              ...
              return (
                <div key={notice.id} onClick={() => setSelectedNotice(notice)} className="px-3 py-2 sm:py-2.5 hover:bg-gray-50 cursor-pointer transition-colors flex items-center justify-between gap-3 group min-w-0">
                  <div className="flex items-center gap-2 min-w-0 flex-1">
                    {notice.isUrgent && (
                      <span className="px-1.5 py-0.5 rounded text-[8px] font-black bg-red-100 text-red-700 flex-shrink-0">緊急</span>
                    )}
                    {cat && <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold border flex-shrink-0 ${cat.color}`}>{cat.name}</span>}
                    
                    <p className={`text-[11px] sm:text-xs font-bold text-gray-900 truncate group-hover:${boardC.text} transition-colors flex-1`}>{notice.title}</p>
                    
                    {notice.attachments && notice.attachments.length > 0 && <Paperclip className="w-3 h-3 text-gray-400 flex-shrink-0" />}
                  </div>
                  
                  <div className="flex items-center gap-1.5 flex-shrink-0">
                    <UserAvatar name={notice.authorName} url={avatarUrl} className="w-4 h-4 sm:w-5 sm:h-5 text-[6px] sm:text-[8px]" />
                    <span className="text-[9px] font-bold text-gray-500 hidden sm:block truncate max-w-[60px]">{notice.authorName?.split(" ")[0]}</span>
                    <span className="text-[9px] font-medium text-gray-400 ml-1 w-8 text-right flex-shrink-0">{dateStr}</span>
                  </div>
                </div>
              );
            })
      ...
      {selectedNotice && (
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 backdrop-blur-sm p-3 sm:p-4 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-3xl overflow-hidden flex flex-col border border-gray-200 max-h-[85vh]">
            
            <div className="p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/80">
              <div className="flex items-center gap-2">
                {selectedNotice.isUrgent && <span className="px-2 py-0.5 rounded-lg text-[9px] font-black bg-red-600 text-white flex items-center shadow-xs"><AlertOctagon className="w-2.5 h-2.5 mr-0.5" />緊急</span>}
                <span className={`px-2 py-0.5 rounded-lg text-[10px] font-bold ${boardC.badgeBg} ${boardC.badgeText}`}>
                  {boardApp.displayName}
                </span>
              ...
              <button onClick={() => setSelectedNotice(null)} className="p-1.5 text-gray-400 hover:bg-gray-200 rounded-lg transition-colors"><X className="h-5 w-5" /></button>
            </div>
            
            <div className="p-5 sm:p-8 overflow-y-auto custom-scrollbar flex-1 flex flex-col">
              <h3 className="text-lg sm:text-xl font-black text-gray-900 mb-4 leading-snug">{selectedNotice.title}</h3>
              
              <div className="flex items-center gap-3 mb-6 bg-gray-50 p-2.5 rounded-xl border border-gray-100 w-fit pr-4">
                <UserAvatar name={selectedNotice.authorName} url={selectedAvatarUrl} className="w-8 h-8 text-[12px]" />
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-gray-800 leading-none">{selectedNotice.authorName}</span>
                  <span className="text-[10px] text-gray-400 font-medium mt-1">掲載: {new Date(selectedNotice.publishStartDate || selectedNotice.createdAt).toLocaleString('ja-JP')}</span>
                </div>
              </div>

              <div 
                className="p-5 bg-white rounded-xl text-sm text-gray-800 whitespace-pre-wrap leading-relaxed border border-gray-200 shadow-inner min-h-[120px] overflow-hidden break-words [&_a]:text-blue-600 [&_a]:underline [&_b]:font-black [&_i]:italic [&_u]:underline [&_font[size='2']]:text-xs [&_font[size='3']]:text-sm [&_font[size='5']]:text-xl [&_font[size='7']]:text-3xl [&_span[style*='background-color']]:px-1.5 [&_span[style*='background-color']]:py-0.5 [&_span[style*='background-color']]:rounded-md"
                dangerouslySetInnerHTML={{ __html: selectedNotice.content }}
              />

              {selectedNotice.attachments && selectedNotice.attachments.length > 0 && (
                <div className="mt-6 pt-4 border-t border-dashed border-gray-200">
                  <h5 className="text-[11px] font-bold text-gray-500 mb-3 flex items-center"><Paperclip className="w-3.5 h-3.5 mr-1"/> 添付ファイル</h5>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    {selectedNotice.attachments.map((file: any, idx: number) => (
                      <a 
                        key={idx} href={file.url} target="_blank" rel="noopener noreferrer"
                        className="flex items-center justify-between p-3 bg-white border border-gray-200 rounded-xl hover:border-blue-300 hover:bg-blue-50/50 transition-all group shadow-2xs"
                      >
            ...
            <div className="p-4 border-t border-gray-100 bg-gray-50/80 flex justify-end mt-auto">
              <button onClick={() => setSelectedNotice(null)} className="px-6 py-2 bg-gray-900 hover:bg-black transition-colors text-white text-xs font-bold rounded-xl shadow-md">閉じる</button>
            </div>
          </div>
        </div>
      )}
