...
// ★ コンポーネント内で直接型を定義し、isManager を追加（循環参照エラーを回避）
...
export default function UserDetailsModal({ user, schoolData, positions, onClose, showAlert, onUpdateUser }: Props) {
              ...
              <div key={p.id} className={`flex items-center justify-between px-3 py-2 rounded-xl border transition-colors ${hasPos ? 'bg-indigo-50/50 border-indigo-200' : 'bg-white border-gray-100 hover:border-gray-300'}`}>
                <div className="flex items-center gap-2">
                  <label className="flex items-center text-xs font-bold text-gray-800 cursor-pointer">
                    <input type="checkbox" checked={hasPos} onChange={e => togglePosition(p.id, e.target.checked)} className="mr-2 text-indigo-600 rounded cursor-pointer" />
                    {p.name}
                  </label>
                  {isLeader && (
                ...
                {hasPos && (
                  <label className="flex items-center text-[10px] font-bold text-amber-600 cursor-pointer animate-fade-in bg-white px-2 py-0.5 rounded shadow-sm border border-amber-100">
                    <input type="radio" checked={isPrimary} onChange={() => setPrimaryPosition(p.id)} className="mr-1.5 text-amber-500 cursor-pointer" />
                    優先
                  </label>
                )}
              </div>
  ...
  const PolicyRow = ({ methodKey, name, icon: Icon }: { methodKey: "email" | "totp" | "passkey", name: string, icon: any }) => {
    ...
    return (
      <div className={`p-4 rounded-xl border transition-colors ${useCustomMfaPolicy ? 'bg-white border-gray-200' : 'bg-gray-50 border-gray-100 opacity-70'}`}>
        ...
        <div className="grid grid-cols-3 gap-3">
          <label className="flex flex-col gap-1.5 cursor-pointer">
            ...
            <div className={`px-2 py-1.5 rounded text-[10px] font-bold text-center border transition-colors ${policy.allowSetup ? 'bg-indigo-50 border-indigo-200 text-indigo-700' : 'bg-white border-gray-200 text-gray-400'}`}>
          ...
          <label className="flex flex-col gap-1.5 cursor-pointer">
            ...
            <div className={`px-2 py-1.5 rounded text-[10px] font-bold text-center border transition-colors ${policy.forceSetup ? 'bg-red-50 border-red-200 text-red-700' : 'bg-white border-gray-200 text-gray-400'}`}>
          ...
          <label className="flex flex-col gap-1.5 cursor-pointer">
            ...
            <div className={`px-2 py-1.5 rounded text-[10px] font-bold text-center border transition-colors ${policy.allowUsage ? 'bg-green-50 border-green-200 text-green-700' : 'bg-white border-gray-200 text-gray-400'}`}>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="px-6 py-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/80 flex-shrink-0">
          <div>
            <h3 className="text-sm font-black text-gray-900">{editData.name} のアカウント設定</h3>
            <p className="text-[10px] font-bold text-gray-500">{editData.email}</p>
          </div>
        ...
        <div className="flex border-b border-gray-100 flex-shrink-0 bg-white">
          <button onClick={() => setActiveTab("basic")} className={`flex-1 py-3 text-xs font-bold transition-colors ${activeTab === "basic" ? "border-b-2 border-indigo-500 text-indigo-600" : "text-gray-500 hover:bg-gray-50"}`}>
            基本情報・役職
          </button>
          <button onClick={() => setActiveTab("profile")} className={`flex-1 py-3 text-xs font-bold transition-colors ${activeTab === "profile" ? "border-b-2 border-indigo-500 text-indigo-600" : "text-gray-500 hover:bg-gray-50"}`}>
            プロフィール詳細
          </button>
          <button onClick={() => setActiveTab("security")} className={`flex-1 py-3 text-xs font-bold transition-colors ${activeTab === "security" ? "border-b-2 border-indigo-500 text-indigo-600" : "text-gray-500 hover:bg-gray-50"}`}>
            認証・セキュリティ
          </button>
        </div>
            ...
            <div className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={editData.name} onChange={(e) => setEditData({ ...editData, name: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm font-bold outline-none focus:ring-2 focus:ring-indigo-500 bg-white"/>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 mb-1">メールアドレス</label>
                  <input type="text" value={editData.email} disabled className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm font-bold bg-gray-50 text-gray-500"/>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <select value={editData.role} onChange={(e) => setEditData({ ...editData, role: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-sm font-bold outline-none focus:ring-2 focus:ring-indigo-500 bg-white">
              ...
              <div className="mt-2">
                <label className="block text-[10px] font-bold text-gray-500 mb-1 flex items-center justify-between">
                  付与する役職
                  <span className="text-[9px] text-gray-400">チェックで付与 / 優先は画面上の表示名になります</span>
                </label>
          ...
          <div className={activeTab === "profile" ? "block animate-fade-in" : "hidden"}>
            <div className="space-y-5">
              <div className="flex items-center gap-2 mb-4 pb-2 border-b border-gray-100">
                ...
                <h4 className="text-sm font-black text-gray-900">詳細プロフィール情報</h4>
              </div>

              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 mb-1">システム利用番号</label>
                  <input type="text" value={editData.systemId || ""} disabled className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none bg-gray-50 text-gray-500"/>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 mb-1">学籍・教職員番号</label>
                  <input type="text" value={editData.studentId || ""} onChange={(e) => setEditData({ ...editData, studentId: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={editData.nameKana || ""} onChange={(e) => setEditData({ ...editData, nameKana: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] font-bold text-gray-500 mb-1">性別</label>
                    <select value={editData.gender || ""} onChange={(e) => setEditData({ ...editData, gender: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500">
                  ...
                  <div>
                    ...
                    <input type="date" value={editData.birthDate || ""} onChange={(e) => setEditData({ ...editData, birthDate: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-3 gap-4">
                <div>
                  ...
                  <input type="text" value={editData.department || ""} onChange={(e) => setEditData({ ...editData, department: e.target.value })} placeholder="例: 普通科" className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 mb-1">学年 (Grade)</label>
                  <input type="text" value={editData.grade || ""} onChange={(e) => setEditData({ ...editData, grade: e.target.value })} placeholder="例: 2" className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="block text-[10px] font-bold text-gray-500 mb-1">組</label>
                    <input type="text" value={editData.classNumber || ""} onChange={(e) => setEditData({ ...editData, classNumber: e.target.value })} placeholder="例: 4" className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                  </div>
                  <div>
                    <label className="block text-[10px] font-bold text-gray-500 mb-1">出席番号</label>
                    <input type="text" value={editData.attendanceNumber || ""} onChange={(e) => setEditData({ ...editData, attendanceNumber: e.target.value })} placeholder="例: 38" className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={editData.club || ""} onChange={(e) => setEditData({ ...editData, club: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 mb-1">出身校・前所属</label>
                  <input type="text" value={editData.previousSchool || ""} onChange={(e) => setEditData({ ...editData, previousSchool: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="tel" value={editData.phoneNumber || ""} onChange={(e) => setEditData({ ...editData, phoneNumber: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 mb-1">居住地 (都道府県市区町村など)</label>
                  <input type="text" value={editData.organizationAddress || ""} onChange={(e) => setEditData({ ...editData, organizationAddress: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 pt-4 border-t border-gray-100">
                <div>
                  ...
                  <input type="date" value={editData.accountValidStartDate || ""} onChange={(e) => setEditData({ ...editData, accountValidStartDate: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-gray-500 mb-1">アカウント有効期間 (終了/期限)</label>
                  <input type="date" value={editData.accountValidEndDate || ""} onChange={(e) => setEditData({ ...editData, accountValidEndDate: e.target.value })} className="w-full border border-gray-200 rounded-xl px-3 py-2 text-xs font-bold outline-none focus:ring-2 focus:ring-indigo-500"/>
                </div>
              </div>
          ...
          <div className={activeTab === "security" ? "block animate-fade-in" : "hidden"}>
            <div className="space-y-6">
              <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                ...
                <select value={editData.accountStatus} onChange={(e) => setEditData({ ...editData, accountStatus: e.target.value as any })} className="w-full border border-gray-300 rounded-lg px-3 py-2 text-xs font-bold bg-white outline-none focus:ring-2 focus:ring-indigo-500">
              ...
              <div>
                <div className="flex justify-between items-center mb-3">
                  <h4 className="text-sm font-black text-gray-900 flex items-center"><Lock className="h-4 w-4 mr-1.5 text-indigo-600"/> 多要素認証 (MFA) 個人ポリシー設定</h4>
                </div>
                
                <div className="bg-gray-100 p-1 rounded-xl flex mb-4 border border-gray-200">
                  <button onClick={() => setUseCustomMfaPolicy(false)} className={`flex-1 py-1.5 text-[10px] font-bold rounded-lg transition-colors ${!useCustomMfaPolicy ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>テナント組織設定に同期</button>
                  <button onClick={() => setUseCustomMfaPolicy(true)} className={`flex-1 py-1.5 text-[10px] font-bold rounded-lg transition-colors ${useCustomMfaPolicy ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-500 hover:text-gray-700'}`}>個人設定を優先</button>
                </div>

                <div className="space-y-3 relative">
                  {!useCustomMfaPolicy && <div className="absolute inset-0 z-10 bg-white/50 backdrop-blur-[1px] rounded-xl flex items-center justify-center"><span className="text-xs font-bold text-gray-600 bg-white px-4 py-2 rounded-lg shadow-sm border border-gray-200">テナントの組織設定に同期中です</span></div>}
        ...
        <div className="px-6 py-4 border-t border-gray-100 bg-gray-50 flex justify-end flex-shrink-0 gap-2">
          <button onClick={onClose} disabled={isUpdating} className="px-5 py-2.5 bg-white border border-gray-300 text-gray-700 rounded-xl text-xs font-bold hover:bg-gray-100 transition-colors disabled:opacity-50">キャンセル</button>
          <button onClick={handleSave} disabled={isUpdating} className="px-6 py-2.5 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 shadow-sm transition-colors flex items-center disabled:opacity-50">
