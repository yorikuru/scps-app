...
export default function CsvUploadModal({ schoolData, users, positions, fetchUsers, onClose, showAlert, onNavigateTab }: Props) {
  ...
  if (positions.length === 0) {
    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
        <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm sm:max-w-md overflow-hidden p-5 sm:p-6 text-center">
          <AlertCircle className="w-10 h-10 sm:w-12 sm:h-12 text-red-500 mx-auto mb-3 sm:mb-4" />
          <h3 className="text-sm sm:text-lg font-black text-gray-900 mb-1.5 sm:mb-2">役職が設定されていません</h3>
          <p className="text-[10px] sm:text-xs font-bold text-gray-500 mb-4 sm:mb-6 leading-relaxed">
            CSVでユーザーを一括登録する前に、このテナントで使用する「役職マスタ」を設定する必要があります。
          </p>
          <div className="flex flex-col gap-2">
            <button onClick={() => { onClose(); onNavigateTab("positions"); }} className="w-full py-2.5 sm:py-3 bg-indigo-600 hover:bg-indigo-700 text-white text-[11px] sm:text-xs font-bold rounded-xl shadow-sm transition-colors">
              役職マスタを設定する
            </button>
            <button onClick={onClose} className="w-full py-2.5 sm:py-3 bg-white border border-gray-200 text-gray-600 text-[11px] sm:text-xs font-bold rounded-xl hover:bg-gray-50 transition-colors">
              キャンセル
            </button>
          </div>
        </div>
  ...
  // CustomSelect に合わせた options フォーマット関数
  const getPositionOptions = (role: string) => {
    ...
    const options = [{ value: "", label: "役職なし" }];
    
    if (showStudent && sortedStudents.length > 0) {
      options.push({ value: "disabled_student", label: "--- 生徒の役職 ---" });
      sortedStudents.forEach(p => options.push({ value: p.id, label: `  ${p.name}` }));
    }
    if (showTeacher && sortedTeachers.length > 0) {
      options.push({ value: "disabled_teacher", label: "--- 教職員・管理職 ---" });
      sortedTeachers.forEach(p => options.push({ value: p.id, label: `  ${p.name}` }));
    }
    if (sortedExternals.length > 0) {
      options.push({ value: "disabled_external", label: "--- 外部組織等 ---" });
      sortedExternals.forEach(p => options.push({ value: p.id, label: `  ${p.name}` }));
    }
    return options;
  };
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-white rounded-2xl shadow-xl w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="p-3 sm:p-4 border-b border-gray-100 flex justify-between items-center bg-gray-50/80 flex-shrink-0">
          <div className="flex items-center">
            <div className="bg-indigo-100 p-1.5 sm:p-2 rounded-lg sm:rounded-xl mr-2.5 sm:mr-3"><FileSpreadsheet className="h-4 w-4 sm:h-5 sm:w-5 text-indigo-600" /></div>
            <div>
              <h3 className="text-xs sm:text-sm font-black text-gray-900">アカウント一括登録 (CSV)</h3>
              <p className="text-[9px] sm:text-[10px] font-bold text-gray-500 mt-0.5">Google Workspace 等から書き出したCSVをそのまま取り込めます</p>
            </div>
          </div>
          <button onClick={onClose} className="p-1 sm:p-1.5 hover:bg-gray-200 rounded-lg text-gray-400 transition-colors"><X className="h-4 w-4 sm:h-5 sm:w-5" /></button>
        </div>
          ...
          {step === 1 ? (
            <div className="space-y-4 sm:space-y-6">
              <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 sm:p-4 flex gap-2 sm:gap-3">
                ...
                <div className="flex-1">
                  <h4 className="text-[11px] sm:text-xs font-black text-blue-900 mb-1">CSVファイルのフォーマットについて</h4>
                  <p className="text-[9px] sm:text-[10px] font-bold text-blue-800/80 leading-relaxed mb-3">
                  ...
                  <button onClick={downloadTemplate} className="inline-flex items-center px-2.5 sm:px-3 py-1.5 bg-white border border-blue-200 text-blue-700 text-[9px] sm:text-[10px] font-bold rounded-lg shadow-sm hover:bg-blue-50 transition-colors">
                    <Download className="w-3 h-3 sm:w-3.5 sm:h-3.5 mr-1.5"/> テンプレートをダウンロード
                  </button>
                </div>
              </div>
              ...
              <div className="text-center">
                ...
                <button onClick={() => fileInputRef.current?.click()} disabled={isUploading} className="w-full flex flex-col items-center justify-center p-8 sm:p-10 border-2 border-dashed border-gray-300 rounded-2xl hover:bg-indigo-50 hover:border-indigo-400 transition-all group disabled:opacity-50">
                  {isUploading ? (
                    <><Loader2 className="h-8 w-8 sm:h-10 sm:w-10 text-indigo-500 animate-spin mb-3" /><span className="text-[11px] sm:text-xs font-bold text-gray-600">読み込み中...</span></>
                  ) : (
                    <><div className="bg-indigo-100 p-3 sm:p-4 rounded-full mb-3 group-hover:scale-110 transition-transform"><Upload className="h-6 w-6 sm:h-8 sm:w-8 text-indigo-600" /></div><span className="text-xs sm:text-sm font-black text-indigo-700 mb-1">ファイルを選択してアップロード</span></>
                  )}
                </button>
              </div>
            </div>
            ...
            <div className="space-y-3 sm:space-y-4">
              <div className="flex items-center justify-between">
                <h4 className="text-xs sm:text-sm font-black text-gray-900">プレビューと役職の設定 ({previewData.length}件)</h4>
              </div>

              <div className="p-2 sm:p-3 bg-gray-50 rounded-xl border border-gray-200 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                <span className="text-[10px] sm:text-xs font-bold text-gray-700">追加するユーザー全員に同じ役職を付与する：</span>
                <div className="w-full sm:w-[200px]">
                  <CustomSelect 
                    value={bulkPositionId} 
                    onChange={val => {
                      if (val.startsWith('disabled_')) return;
                      handleBulkPositionChange(val);
                    }} 
                    options={getPositionOptions("all")}
                    buttonClassName="w-full text-[10px] sm:text-[11px] font-bold border border-gray-300 px-2 sm:px-3 py-1.5 rounded-lg outline-none focus:ring-2 focus:ring-indigo-500 bg-white shadow-sm"
                  />
                </div>
              </div>
                ...
                <div className="overflow-x-auto custom-scrollbar">
                  <table className="min-w-full divide-y divide-gray-200 text-left whitespace-nowrap">
                    <thead className="bg-gray-50">
                      <tr>
                        <th className="px-2 sm:px-4 py-1.5 sm:py-2 text-center w-8 sm:w-10">
                          <input 
                            type="checkbox" 
                            checked={isAllSelected}
                            onChange={(e) => toggleAllSelection(e.target.checked)}
                            className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-indigo-600 focus:ring-indigo-500 rounded cursor-pointer"
                          />
                        </th>
                        <th className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-black text-gray-500">状態</th>
                        <th className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-black text-gray-500">名前</th>
                        <th className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-black text-gray-500">メールアドレス</th>
                        <th className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-black text-gray-500">権限</th>
                        <th className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-black text-gray-500 text-indigo-600">発行パスワード</th>
                        <th className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-black text-gray-500">付与する役職 (優先)</th>
                        <th className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-black text-gray-500">システムID</th>
                        <th className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-black text-gray-500">学籍/教番</th>
                        <th className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-black text-gray-500">所属</th>
                      </tr>
                    </thead>
                        ...
                        <tr key={i} className={`${row._error ? "bg-red-50" : row._selected ? "bg-indigo-50/20" : "bg-white"} hover:bg-gray-50`}>
                          <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-center">
                            <input 
                              ...
                              className="w-3 h-3 sm:w-3.5 sm:h-3.5 text-indigo-600 focus:ring-indigo-500 rounded cursor-pointer disabled:opacity-30"
                            />
                          </td>
                          <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-bold">
                          ...
                          <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-[10px] sm:text-xs font-black text-gray-900">
                            {row._nameCandidates && row._nameCandidates.length > 0 ? (
                              <CustomSelect 
                                value={row._selectedName || ""} 
                                onChange={(val) => updateNameSelection(i, val)}
                                options={[
                                  { value: "", label: "スペース位置を選択" },
                                  ...(row._nameCandidates || []).map(c => ({ value: c, label: c }))
                                ]}
                                buttonClassName={`w-full border ${row._selectedName ? 'border-gray-200' : 'border-red-400 bg-red-50'} rounded-md px-2 py-1 text-[9px] sm:text-[10px] font-bold outline-none focus:ring-1 focus:ring-indigo-500`}
                              />
                            ) : (
                              row.name
                            )}
                          </td>
                          <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] font-bold text-gray-500">{row.email}</td>
                          <td className="px-2 sm:px-4 py-1.5 sm:py-2">
                            <span className={`px-1.5 sm:px-2 py-0.5 rounded text-[8px] sm:text-[9px] font-bold ${row.role==='admin'?'bg-red-100 text-red-700':row.role==='teacher'?'bg-purple-100 text-purple-700':row.role==='officer'?'bg-indigo-100 text-indigo-700':'bg-gray-100 text-gray-700'}`}>
                              {row.role}
                            </span>
                          </td>
                          <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-[10px] sm:text-xs font-mono font-bold text-red-500">
                            {row.initialPassword || "-"}
                          </td>
                          <td className="px-2 sm:px-4 py-1.5 sm:py-2 min-w-[140px]">
                            <CustomSelect 
                              value={row._primaryPositionId || ""} 
                              onChange={(val) => {
                                if (val.startsWith('disabled_')) return;
                                updateRowPosition(i, val);
                              }} 
                              disabled={!row._selected || !!row._error} 
                              options={getPositionOptions(row.role)}
                              buttonClassName="w-full border border-gray-200 rounded-md px-2 py-1 text-[9px] sm:text-[10px] font-bold outline-none focus:ring-1 focus:ring-indigo-500 bg-white shadow-2xs disabled:opacity-50"
                            />
                          </td>
                          <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] text-gray-500 font-mono">{row.systemId || "-"}</td>
                          <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] text-gray-500">{row.studentId || "-"}</td>
                          <td className="px-2 sm:px-4 py-1.5 sm:py-2 text-[9px] sm:text-[10px] text-gray-500">{row.department || "-"}</td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
              ...
              {previewData.length > 100 && <p className="text-[9px] sm:text-[10px] text-gray-400 text-center mt-1 sm:mt-2 font-bold">※プレビューは100件まで表示されますが、登録は選択された全件実行されます。</p>}
            </div>
          )}
        </div>

        {step === 2 && (
          <div className="p-3 sm:p-4 border-t border-gray-100 bg-gray-50 flex justify-between items-center flex-shrink-0">
            <div className="text-[9px] sm:text-[10px] font-bold text-gray-500">
              選択中: <span className="text-indigo-600 font-black text-[11px] sm:text-xs">{previewData.filter(r => r._selected).length}</span> 件
            </div>
            <div className="flex gap-2">
              <button onClick={() => setStep(1)} disabled={isUploading} className="px-4 sm:px-5 py-1.5 sm:py-2 border border-gray-300 rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-bold text-gray-700 bg-white hover:bg-gray-100 transition-colors shadow-2xs">戻る</button>
              <button onClick={handleExecuteUpload} disabled={isUploading || previewData.filter(r => r._selected).length === 0} className="px-4 sm:px-6 py-1.5 sm:py-2 border border-transparent rounded-lg sm:rounded-xl text-[10px] sm:text-xs font-bold text-white bg-indigo-600 hover:bg-indigo-700 flex items-center transition-colors shadow-sm disabled:opacity-50">
                {isUploading ? <><Loader2 className="animate-spin h-3.5 w-3.5 sm:h-4 sm:w-4 mr-1.5" /> 登録中...</> : <><CheckCircle2 className="h-3.5 w-3.5 sm:h-4 sm:w-4 mr-1.5" /> 選択したユーザーを登録</>}
              </button>
            </div>
          </div>
        )}
