...
export default function MessageForm({ schoolData, users, currentUser, showAlert, editMessage, onSuccess, onCancel }: Props) {
  ...
  const executeSave = async () => {
    ...
    try {
      ...
      targets.forEach(uid => {
        // ★ 削除: if (uid === currentUser.id) return;
      ...
      if (isEdit) {
        showAlert("success", "更新しました。全対象ユーザーのダッシュボードに再表示されます。");
      } else {
          ...
          <div className="lg:col-span-8 space-y-6">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 space-y-6">
              ...
              <div>
                ...
                <input type="text" required value={title} onChange={e => setTitle(e.target.value)} className="w-full border border-gray-300 rounded-xl py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none transition-all shadow-xs" placeholder="例: 全体システムメンテナンスのお知らせ" />
              </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 space-y-5">
              ...
              <div className="flex flex-wrap gap-3 p-1.5 bg-gray-50 rounded-xl border border-gray-200 w-fit">
                <label className={`flex items-center px-4 py-2 rounded-lg cursor-pointer transition-colors ${targetType === "tenant" ? "bg-white shadow-sm border border-gray-200 font-bold text-blue-700" : "text-gray-500 hover:text-gray-700 font-medium"}`}>
                  ...
                  <Globe className={`w-4 h-4 mr-2 ${targetType === "tenant" ? "text-blue-600" : "opacity-50"}`} /> テナント全体
                </label>
                    ...
                    <div className="pt-3 border-t border-gray-200">
                      ...
                      <div className="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto custom-scrollbar p-2 bg-white border border-gray-200 rounded-lg">
                        {departmentUserIds.map(uid => {
                          ...
                          return <span key={uid} className="text-[10px] font-bold text-gray-700 bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200">{u?.name || "不明"}</span>;
                        })}
                      </div>
                    </div>
                  )}
              ...
              {targetType === "user" && (
                <div className="border border-gray-200 rounded-xl overflow-hidden bg-gray-50/50 animate-fade-in">
                  <div className="p-3 border-b border-gray-200 bg-white flex justify-between items-center">
                    ...
                    <div className="text-xs font-bold text-indigo-700 bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100">選択中: {targetIds.length}名</div>
                  </div>
                    ...
                    {filteredUsers.map(u => (
                      <label key={u.id} className={`flex items-center p-2.5 rounded-xl cursor-pointer transition-all border ${targetIds.includes(u.id) ? 'bg-blue-50 border-blue-200 shadow-sm' : 'hover:bg-gray-50 border-transparent'}`}>
                        ...
                        <div className="flex flex-col min-w-0">
                          <span className="text-sm text-gray-900 font-bold truncate">{u.name}</span>
                          <span className="text-[10px] text-gray-500 truncate">{u.email || "メール未設定"}</span>
                        </div>
                      </label>
                    ))}
                    {filteredUsers.length === 0 && <div className="col-span-full p-6 text-center text-sm font-bold text-gray-400">一致するユーザーがいません</div>}
                  </div>
                </div>
              )}
            </div>
