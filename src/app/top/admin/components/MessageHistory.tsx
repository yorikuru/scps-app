...
export default function MessageHistory({ messages, users, setQueryParams, requestDelete }: Props) {
  ...
  return (
    <div className="bg-white rounded-b-2xl shadow-sm border border-gray-200 flex flex-col h-full min-h-0">
      ...
      <div className="p-3 sm:p-4 border-b border-gray-200 bg-gray-50/50 space-y-3 shrink-0">
        <div className="flex flex-col md:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input 
              ...
              className="w-full pl-9 pr-3 py-2 text-[16px] sm:text-xs font-bold border border-gray-300 rounded-lg focus:ring-blue-500 focus:border-blue-500 outline-none"
            />
          </div>

          <div className="flex gap-2 sm:gap-3 overflow-x-auto custom-scrollbar pb-1 md:pb-0 snap-x">
            <select value={categoryFilter} onChange={e => setCategoryFilter(e.target.value)} className="border border-gray-300 rounded-lg py-2 px-2 sm:px-3 text-[16px] sm:text-xs font-bold bg-white outline-none focus:ring-blue-500 shrink-0 snap-start">
            ...
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} className="border border-gray-300 rounded-lg py-2 px-2 sm:px-3 text-[16px] sm:text-xs font-bold bg-white outline-none focus:ring-blue-500 shrink-0 snap-start">
              <option value="all">全状態</option>
              <option value="active">配信中</option>
              <option value="inactive">期間外</option>
            </select>

            <select value={responseFilter} onChange={e => setResponseFilter(e.target.value)} className="border border-gray-300 rounded-lg py-2 px-2 sm:px-3 text-[16px] sm:text-xs font-bold bg-white outline-none focus:ring-blue-500 shrink-0 snap-start">
            ...
            <div className="flex items-center gap-1.5 px-1 shrink-0 snap-start">
              ...
              <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="border border-gray-300 rounded-lg py-2 px-2 sm:px-3 text-[16px] sm:text-xs font-bold bg-white outline-none focus:ring-blue-500">
                <option value="start_desc">開始日: 新</option>
                <option value="start_asc">開始日: 古</option>
                <option value="create_desc">作成日: 新</option>
                <option value="create_asc">作成日: 古</option>
              </select>
            </div>
          </div>
        </div>
      ...
      <div className="flex-1 overflow-x-auto overflow-y-auto custom-scrollbar min-h-0 bg-white">
        {messages.length === 0 ? (
          ...
          <table className="min-w-full divide-y divide-gray-200 text-left whitespace-nowrap">
            <thead className="bg-gray-50 text-[10px] sm:text-[11px] font-bold text-gray-500 sticky top-0 z-10 shadow-sm">
              <tr>
                <th className="px-3 sm:px-4 py-3 border-r border-gray-100 w-24 sm:w-28">状態 / カテゴリ</th>
                <th className="px-3 sm:px-4 py-3 border-r border-gray-100 min-w-[200px] sm:min-w-[250px]">タイトル / 宛先</th>
                <th className="px-3 sm:px-4 py-3 border-r border-gray-100 w-40 sm:w-48">掲載期間</th>
                <th className="px-3 sm:px-4 py-3 border-r border-gray-100 w-32 sm:w-40 text-center">対応要求</th>
                <th className="px-3 sm:px-4 py-3 w-20 sm:w-24 text-right">操作</th>
              </tr>
            </thead>
              ...
              {processedMessages.map(msg => {
                ...
                return (
                  <tr key={msg.id} onClick={() => setQueryParams({ viewId: msg.id })} className="hover:bg-blue-50/50 cursor-pointer transition-colors group">
                    <td className="px-3 sm:px-4 py-3 border-r border-gray-100 align-top">
                    ...
                    <td className="px-3 sm:px-4 py-3 border-r border-gray-100 whitespace-normal">
                      <div className="flex flex-col gap-1.5">
                        ...
                        <div className="flex items-center gap-1.5 flex-wrap">
                          ...
                          <span className="px-1.5 py-0.5 bg-gray-100 text-gray-600 text-[8px] sm:text-[9px] font-bold rounded border border-gray-200 leading-tight inline-block max-w-xs truncate">
                    ...
                    <td className="px-3 sm:px-4 py-3 border-r border-gray-100 text-[9px] sm:text-[10px] font-medium text-gray-500 align-top">
                      <div className="flex flex-col gap-1">
                        <span className="flex items-center"><Calendar className="w-2.5 h-2.5 sm:w-3 sm:h-3 mr-1"/>開始: {msg.startAt ? msg.startAt.replace('T', ' ') : '指定なし'}</span>
                        <span className="flex items-center"><Calendar className="w-2.5 h-2.5 sm:w-3 sm:h-3 mr-1 opacity-50"/>終了: {msg.endAt ? msg.endAt.replace('T', ' ') : '指定なし'}</span>
                      </div>
                    </td>

                    <td className="px-3 sm:px-4 py-3 border-r border-gray-100 text-center align-top">
                      {msg.requireResponse ? (
                        isCompleted ? (
                          <div className="flex flex-col items-center gap-0.5">
                            <span className="px-2 py-1 bg-blue-100 text-blue-800 text-[9px] sm:text-[10px] font-black rounded border border-blue-200 flex items-center">
                              <CheckSquare className="w-2.5 h-2.5 sm:w-3 sm:h-3 mr-1"/> {msg.responseType === "single" ? "対応完了" : "全員完了"}
                            </span>
                            {msg.responseType === "single" && msg.responses && msg.responses.length > 0 && (
                              <span className="text-[8px] sm:text-[9px] text-gray-500 font-bold truncate max-w-[80px] sm:max-w-[100px]">
                          ...
                          <div className="flex flex-col items-center gap-0.5">
                            <span className="text-[9px] sm:text-[10px] font-bold text-red-700 bg-red-50 border border-red-100 px-1.5 py-0.5 rounded">
                    ...
                    <td className="px-3 sm:px-4 py-3 text-right align-top">
                      {!isSystemMsg ? (
                        <div className="flex justify-end gap-1 sm:gap-1.5">
                          <button onClick={(e) => { e.stopPropagation(); setQueryParams({ tab: "form", editId: msg.id }); }} className="p-1 sm:p-1.5 text-blue-600 hover:bg-blue-100 rounded-md transition-colors"><Edit2 className="w-3.5 h-3.5 sm:w-4 sm:h-4"/></button>
                          <button onClick={(e) => { e.stopPropagation(); requestDelete(msg.id); }} className="p-1 sm:p-1.5 text-red-600 hover:bg-red-100 rounded-md transition-colors"><Trash2 className="w-3.5 h-3.5 sm:w-4 sm:h-4"/></button>
                        </div>
                      ) : (
                        <span className="text-[8px] sm:text-[9px] text-gray-400 font-bold bg-gray-50 px-1.5 sm:px-2 py-1 rounded border border-gray-100">
                          参照のみ
                        </span>
                      )}
                    </td>
