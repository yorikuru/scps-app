  ...
  const isTargetUser = (a: Announcement) => {
    ...
    if (isExtUser) {
      ...
      if (a.targetType === "position") {
        ...
        if (pIds.includes("sys_admin") && (u.role === "admin" || u.role === "system_admin" || (u as any).isITManager)) return true;
        if (pIds.includes("sys_student") && u.role === "student") return true;
        if (pIds.includes("sys_teacher") && u.role === "teacher") return true;
        if ((u as any).positionIds?.some((pid:string) => pIds.includes(pid))) return true;
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar flex flex-col h-full bg-white relative pb-6 sm:pb-0">
        <div className="lg:hidden flex items-center px-4 py-3 border-b border-gray-100 bg-white sticky top-0 z-10 shrink-0">
          <button 
            ...
            className="flex items-center text-sm font-bold text-gray-500 hover:text-gray-900 transition-colors bg-gray-50 px-2 py-1 rounded-lg border border-gray-200"
          >
        ...
        <div className="p-4 sm:p-6 border-b border-gray-100 shrink-0">
          ...
          <div className="flex items-center justify-between flex-wrap gap-4">
            <div className="flex items-center gap-3 bg-gray-50 p-2.5 rounded-xl border border-gray-100 shadow-2xs">
              ...
              <div>
                ...
                <div className="flex items-center gap-1.5 text-[10px] font-bold text-gray-400 mt-0.5">
                  <Calendar className="w-3 h-3" /> 掲載: {new Date(selectedAnnouncement.publishStartDate || selectedAnnouncement.createdAt).toLocaleString('ja-JP', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}
                </div>
              </div>
            </div>
          </div>
        ...
        {!isExternalTab && (
          <div className="p-4 sm:p-6 border-t border-gray-100 bg-white sm:bg-gray-50/30 mt-auto shrink-0 z-10 sticky bottom-0">
            <div className="flex flex-col sm:flex-row gap-4 items-stretch justify-center max-w-lg mx-auto">
              
              {/* 既読ブロック */}
              <div className="flex-1 bg-white rounded-xl p-3 flex items-center justify-between border border-gray-200 shadow-sm">
                <span className="text-[10px] font-black text-gray-400 uppercase tracking-wider shrink-0 mr-2">確認状況</span>
                {isRead ? (
                  <div className="flex items-center justify-end gap-1.5">
                    <span className="text-xs font-black text-emerald-600 flex items-center gap-1.5 bg-emerald-50 px-3 py-1.5 rounded-lg border border-emerald-100 shadow-2xs">
                      <Check className="w-4 h-4" /> 確認済み
                    </span>
                    <button onClick={() => toggleReadStatus(false)} className="p-1.5 text-gray-400 hover:bg-gray-100 rounded-lg transition-colors border border-transparent hover:border-gray-200" title="未読に戻す">
                      <XCircle className="w-4 h-4" />
                    </button>
                  </div>
                ) : (
                  <button onClick={() => toggleReadStatus(true)} className={`w-full sm:w-auto px-4 py-2 text-white rounded-lg text-xs font-black shadow-md transition-transform hover:-translate-y-0.5 flex items-center justify-center gap-2 ${c.bg} ${c.hover}`}>
                    <BookOpen className="w-4 h-4" /> 既読にする
                  </button>
                )}
              </div>
              ...
              {selectedAnnouncement.requireAction && (
                <div className="flex-1 bg-white rounded-xl p-3 flex items-center justify-between border border-indigo-200 shadow-sm relative overflow-hidden">
                  {selectedAnnouncement.actionType === "single" && <div className="absolute top-0 right-0 bg-indigo-500 text-white text-[8px] font-black px-2 py-0.5 rounded-bl-lg shadow-sm">1人対応で完了</div>}
                  <span className="text-[10px] font-black text-indigo-400 uppercase tracking-wider shrink-0 mr-2">タスク</span>
                  
                  {isActioned ? (
                    <div className="flex items-center justify-end gap-1.5">
                      <span className="text-xs font-black text-indigo-600 flex items-center gap-1.5 bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-200 shadow-2xs">
                        <CheckSquare className="w-4 h-4"/> 対応済み
                      </span>
                      <button onClick={() => toggleActionStatus(false)} className="p-1.5 text-indigo-300 hover:bg-indigo-50 hover:text-indigo-500 rounded-lg transition-colors border border-transparent hover:border-indigo-100" title="未対応に戻す">
                        <XCircle className="w-4 h-4"/>
                      </button>
                    </div>
                  ) : (isTaskCompletedBySomeone ? (
                    <span className="text-xs font-black text-gray-500 flex items-center gap-1.5 bg-gray-50 px-3 py-1.5 rounded-lg border border-gray-200 shadow-2xs">
                      <CheckSquare className="w-4 h-4"/> メンバーが対応済
                    </span>
                  ) : (
                    <button onClick={() => toggleActionStatus(true)} className="w-full sm:w-auto px-4 py-2 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-black shadow-md transition-transform hover:-translate-y-0.5 flex items-center justify-center gap-2">
                      <CheckSquare className="w-4 h-4" /> 対応済にする
                    </button>
                  ))}
                </div>
              )}
  ...
  return (
    <div className="flex flex-col lg:flex-row h-full w-full bg-white relative overflow-hidden">
      ...
      <div className={`w-full lg:w-[420px] xl:w-[480px] border-r border-gray-200 flex flex-col flex-shrink-0 bg-white h-full ${showMobileDetail ? 'hidden lg:flex' : 'flex'}`}>
        ...
        <div className="p-2.5 border-b border-gray-200 bg-gray-50/50 flex flex-col gap-2 shrink-0">
          ...
          <div className="flex gap-1.5 overflow-x-auto custom-scrollbar pb-1 text-[10px]">
            <div className="flex-1 min-w-[100px] shrink-0">
              <CustomSelect 
                ...
                buttonClassName="w-full flex items-center justify-between bg-white border border-gray-200 rounded-md px-2 py-1.5 text-gray-700 font-bold shadow-2xs focus:ring-2 focus:ring-indigo-500 text-[10px]"
              />
            </div>
            <div className="flex-1 min-w-[100px] shrink-0">
              <CustomSelect 
            ...
            <button onClick={() => setFilterRequireAction(!filterRequireAction)} className={`p-1.5 rounded-md transition-colors flex items-center justify-center shrink-0 shadow-2xs ${filterRequireAction ? 'bg-indigo-50 border border-indigo-200 text-indigo-600' : 'bg-white border border-gray-200 text-gray-400 hover:bg-gray-50'}`} title="要対応のみ表示">
              <CheckSquare className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setFilterUrgent(!filterUrgent)} className={`p-1.5 rounded-md transition-colors flex items-center justify-center shrink-0 shadow-2xs ${filterUrgent ? 'bg-red-50 border border-red-200 text-red-600' : 'bg-white border border-gray-200 text-gray-400 hover:bg-gray-50'}`} title="緊急のみ表示">
              <AlertOctagon className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setSortOrder(sortOrder === "desc" ? "asc" : "desc")} className="p-1.5 bg-white border border-gray-200 text-gray-400 rounded-md hover:bg-gray-50 transition-colors flex items-center justify-center shrink-0 shadow-2xs" title="並び替え">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar divide-y divide-gray-100">
          {filteredAndSorted.length === 0 ? (
            <div className="flex flex-col items-center justify-center text-gray-400 py-12 opacity-70">
              ...
              <p className="text-[10px] sm:text-xs font-bold">該当する連絡はありません</p>
            </div>
          ) : (
            filteredAndSorted.map((a) => {
              ...
              return (
                <div 
                  ...
                  className={`px-3 py-2.5 cursor-pointer flex items-center gap-2.5 min-w-0 transition-colors group ${isSelected && window.innerWidth >= 1024 ? (isUr ? 'bg-red-600 text-white' : (isExternalTab ? 'bg-blue-600 text-white shadow-inner' : `${c.bg} text-white shadow-inner`)) : 'hover:bg-gray-50 text-gray-900'}`}
                >
                  <div className="relative flex-shrink-0">
                    <UserAvatar name={a.authorName} url={avatarUrl} className={`w-8 h-8 text-[10px]`} />
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-center">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <span className={`text-[10px] truncate max-w-[90px] ${isSelected && window.innerWidth >= 1024 ? 'text-white/80' : 'text-gray-500'}`}>{a.authorName}</span>
                      {cat && <span className={`px-1.5 py-0.5 rounded text-[9px] font-bold border truncate max-w-[60px] ${isSelected && window.innerWidth >= 1024 ? 'border-white/30 text-white/90' : cat.color}`}>{cat.name}</span>}
                    </div>
                  </div>

                  <div className={`w-12 text-right flex-shrink-0 text-[10px] font-medium flex flex-col items-end gap-1.5 ${isSelected && window.innerWidth >= 1024 ? 'text-white/90' : 'text-gray-400'}`}>
