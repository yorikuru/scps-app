    ...
    <div className="space-y-4">
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-8 sm:pl-9 pr-3 py-2 bg-gray-50 border border-gray-200 rounded-xl text-[16px] sm:text-xs font-bold text-gray-900 focus:bg-white outline-none focus:ring-2 focus:ring-indigo-500 transition-colors"
            />
          </div>
          ...
          <div className="flex gap-2 overflow-x-auto custom-scrollbar">
            ...
            >
              ...
              {(Object.keys(PRESENCE_CONFIG) as PresenceState[]).map((st) => (
                <option key={st} value={st}>{PRESENCE_CONFIG[st].label}</option>
              ))}
            </select>
            ...
            <div className="flex items-center gap-1 bg-gray-50 border border-gray-200 rounded-xl px-2 shrink-0">
              ...
              <select
                ...
                className="bg-transparent text-[16px] sm:text-xs font-bold text-gray-700 outline-none py-2"
              >
                <option value="systemId">番号順</option>
                <option value="name">名前順</option>
                <option value="status">状態順</option>
              </select>
              <button 
                type="button" onClick={() => setSortOrder(p => p === 'asc' ? 'desc' : 'asc')}
                className="text-[10px] font-black text-indigo-600 px-1 hover:bg-gray-200 rounded whitespace-nowrap"
              >
                {sortOrder === 'asc' ? '昇順' : '降順'}
              </button>
            </div>
      ...
      <div className="flex items-center justify-between px-1 print:hidden">
        <span className="text-[10px] sm:text-xs font-bold text-gray-500">
          該当メンバー: <span className="text-gray-900 font-black">{processedList.length}名</span> （連絡可能: <span className="text-emerald-600 font-black">{availableCount}名</span>）
        </span>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2.5 sm:gap-4 print:hidden">
        {processedList.map(({ user, presence }) => {
          ...
          return (
            <div key={user.id} className="bg-white p-3 sm:p-4 rounded-xl sm:rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow flex flex-col gap-2.5 min-w-0 relative overflow-visible">
              
              <div className="flex items-start sm:gap-3 gap-2.5 min-w-0">
                <div className="relative shrink-0 mt-0.5">
                  {user.photoURL ? (
                    <img src={user.photoURL} alt={user.name} className="w-10 h-10 sm:w-12 sm:h-12 rounded-full object-cover border border-gray-100" />
                  ) : (
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-full bg-gradient-to-tr from-gray-200 to-gray-300 flex items-center justify-center text-gray-600 font-bold text-lg">
                      {user.name.charAt(0)}
                    </div>
                  )}
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-1.5 mb-0.5 pr-8">
                    <span className="text-[10px] font-mono font-bold text-gray-400">#{(user as any).systemId || '-'}</span>
                    <span className="text-xs sm:text-sm font-black text-gray-900 truncate">{user.name}</span>
                    {isMe && <span className="text-[8px] font-black bg-gray-100 text-gray-600 px-1.5 py-0.5 rounded-md shrink-0">あなた</span>}
                  </div>
                  <span className="text-[9px] sm:text-[10px] font-bold text-gray-500 block truncate">
                  ...
                  <div className="flex items-center gap-1.5 mt-1 sm:mt-1.5">
                    ...
                    {locName && (
                      <>
                        ...
                        <span className="text-[9px] font-bold text-gray-600 flex items-center gap-0.5 truncate"><MapPin className="w-2.5 h-2.5 text-gray-400 shrink-0" /> {locName}</span>
                      </>
                    )}
                  </div>
                </div>
                ...
                <div className="absolute top-3 right-3 flex items-center gap-1">
                  ...
                  {!isMe && (
                    <button onClick={() => router.push(`/top/chat?directUser=${user.id}`)} className="p-1.5 bg-white border border-gray-200 hover:bg-indigo-50 hover:text-indigo-600 hover:border-indigo-200 rounded-lg text-gray-600 transition-colors shadow-2xs" title="チャットを送る">
