...
import { getDefaultChatPermissions } from "@/app/top/chat/types"; // ★ 権限を正確に判定するためインポート
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (mode === "create") {
        ...
        if (!canEdit) return showAlert("編集権限がありません。","error"); // 念のためのガード
      ...
      <div className="px-4 sm:px-6 py-3.5 sm:py-4 border-b border-gray-200 bg-gray-50/80 flex justify-between items-center sticky top-0 z-20 backdrop-blur-sm">
        ...
        <div className="flex items-center gap-1.5 sm:gap-2">
          {mode === "view" && targetUser && (
            <>
              {canEdit && <button onClick={() => setMode("edit")} className="px-2.5 sm:px-3 py-1.5 bg-indigo-50 text-indigo-600 border border-indigo-200 hover:bg-indigo-100 text-[10px] sm:text-xs font-bold rounded-lg flex items-center gap-1 transition-colors"><Edit3 className="w-3.5 h-3.5" /> 編集</button>}
              {canDelete && <button onClick={handleDelete} className="px-2.5 sm:px-3 py-1.5 bg-red-50 text-red-600 border border-red-200 hover:bg-red-100 text-[10px] sm:text-xs font-bold rounded-lg flex items-center gap-1 transition-colors"><Trash2 className="w-3.5 h-3.5" /> 削除</button>}
            </>
          )}
          <button onClick={onClose} className="p-1.5 sm:p-2 text-gray-400 hover:bg-gray-200 rounded-full transition-colors"><X className="w-4 h-4 sm:w-5 sm:h-5" /></button>
        </div>
      </div>
          ...
          {generatedAccount && (
            <div className="bg-amber-50/80 border border-amber-200 rounded-2xl p-3 sm:p-4 shadow-sm animate-fade-in">
              <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2 mb-3">
                ...
                <button onClick={handleCopyCredentials} className="px-3 py-1.5 bg-amber-600 text-white hover:bg-amber-700 rounded-xl text-[10px] font-bold flex items-center justify-center gap-1.5 shadow-sm w-full sm:w-auto transition-colors">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="flex justify-end gap-2 sm:gap-3 pt-4 border-t border-gray-100">
                ...
                <button type="submit" disabled={isSubmitting} className="w-full sm:w-auto px-6 sm:px-8 py-2.5 bg-indigo-600 text-white text-xs sm:text-sm font-bold rounded-xl shadow-md hover:bg-indigo-700 flex items-center justify-center gap-2 disabled:opacity-50">
                  {isSubmitting ? <Loader2 className="w-3.5 h-3.5 sm:w-4 h-4 animate-spin" /> : <Save className="w-3.5 h-3.5 sm:w-4 h-4" />}{mode === "create" ? "仮登録してID発行" : "更新を保存"}
                </button>
              </div>
            </form>
          )}
