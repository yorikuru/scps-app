  ...
  if (!hasPermission) {
    return (
      <div className="h-full flex flex-col items-center justify-center p-6 text-center">
        ...
        <p className="text-sm font-bold text-gray-500">外部ユーザーを閲覧・管理する権限が付与されていません。</p>
      </div>
    );
  }
        ...
        {isSuperAdmin && (
          <div className="flex border-b border-gray-200 mb-3 sm:mb-4 shrink-0 overflow-x-auto custom-scrollbar">
            <button 
              ...
              className={`px-3 sm:px-4 py-2 text-xs sm:text-sm font-bold transition-colors border-b-2 whitespace-nowrap ${activeTab === "list" ? "border-amber-500 text-amber-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
            >
              ゲストユーザー一覧
            </button>
            <button 
              onClick={() => setActiveTab("permissions")}
              className={`px-3 sm:px-4 py-2 text-xs sm:text-sm font-bold transition-colors border-b-2 flex items-center gap-1.5 whitespace-nowrap ${activeTab === "permissions" ? "border-indigo-500 text-indigo-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
            >
              <Shield className="w-3.5 h-3.5 sm:w-4 sm:h-4" /> メンバーの管理権限設定
            </button>
            <button 
              onClick={() => setActiveTab("apps")}
              className={`px-3 sm:px-4 py-2 text-xs sm:text-sm font-bold transition-colors border-b-2 flex items-center gap-1.5 whitespace-nowrap ${activeTab === "apps" ? "border-emerald-500 text-emerald-600" : "border-transparent text-gray-500 hover:text-gray-700"}`}
            >
                ...
                <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-3">
                  <div className="relative w-full lg:max-w-xs">
                    ...
                    <input 
                      ...
                      className="block w-full pl-9 pr-3 py-1.5 sm:py-2 border border-gray-300 rounded-xl text-xs sm:text-sm font-bold bg-white focus:ring-2 focus:ring-indigo-500 outline-none transition-colors" 
                    />
                  </div>

                  <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                    <span className="text-xs font-bold text-indigo-800 whitespace-nowrap">一括操作:</span>
                    <div className="flex items-center gap-2 w-full sm:w-auto">
                      <select value={bulkTarget} onChange={e => setBulkTarget(e.target.value)} className="flex-1 sm:flex-none px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl text-xs font-bold border border-gray-300 bg-white outline-none focus:ring-2 focus:ring-indigo-500">
                        <option value="">対象グループを選択</option>
                        <option value="all">表示中の全ユーザー ({processedPermUsers.length}名)</option>
                        <option value="manager">マネージャー権限</option>
                        <optgroup label="権限ロール">
                          <option value="role_teacher">教職員</option><option value="role_officer">生徒会役員</option>
                        </optgroup>
                        {availablePositions.length > 0 && <optgroup label="役職別">{availablePositions.map(pos => <option key={pos} value={`pos_${pos}`}>{pos}</option>)}</optgroup>}
                      </select>

                      <select value={bulkTargetItem} onChange={e => setBulkTargetItem(e.target.value)} className="flex-1 sm:flex-none px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl text-xs font-bold border border-gray-300 bg-white outline-none focus:ring-2 focus:ring-indigo-500">
                        <option value="all">ゲスト管理すべての権限</option>
                        {EXTERNAL_PERMISSIONS.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}
                      </select>
                    </div>

                    <div className="flex gap-2 w-full sm:w-auto">
                      <button onClick={() => handleBulkUpdate(true)} disabled={isBulkUpdating || !bulkTarget} className="flex-1 sm:flex-none px-3 sm:px-4 py-1.5 sm:py-2 bg-indigo-600 text-white rounded-xl text-xs font-bold hover:bg-indigo-700 disabled:opacity-50 transition-colors flex items-center justify-center">
                        {isBulkUpdating ? <Loader2 className="w-4 h-4 animate-spin" /> : "一括許可"}
                      </button>
                      <button onClick={() => handleBulkUpdate(false)} disabled={isBulkUpdating || !bulkTarget} className="flex-1 sm:flex-none px-3 sm:px-4 py-1.5 sm:py-2 bg-white border border-gray-300 text-gray-700 rounded-xl text-xs font-bold hover:bg-gray-50 disabled:opacity-50 transition-colors flex items-center justify-center">
                        {isBulkUpdating ? <Loader2 className="w-4 h-4 animate-spin" /> : "一括解除"}
                      </button>
                    </div>
                  </div>
                </div>
              ...
              <div className="flex-1 overflow-auto custom-scrollbar relative">
                <table className="min-w-full divide-y divide-gray-200 text-left whitespace-nowrap">
                  <thead className="bg-gray-50 text-[10px] font-black text-gray-500 sticky top-0 z-10 shadow-2xs">
                    <tr>
                      <th scope="col" className="px-4 py-3 w-24 border-r border-gray-200 bg-gray-50">利用番号</th>
                      <th scope="col" className="px-4 py-3 border-r border-gray-200 min-w-[200px] bg-gray-50">ユーザー名 / 役職</th>
                      <th scope="col" className="px-4 py-3 text-center border-r border-gray-200 w-28 bg-gray-50">一括切替</th>
                      {EXTERNAL_PERMISSIONS.map(item => {
                        const Icon = item.icon;
                        return (
                          <th key={item.id} scope="col" className="px-3 py-2 text-center min-w-[100px] border-r border-gray-100 last:border-0 bg-gray-50">
                            <div className="flex flex-col items-center gap-1.5"><Icon className="w-4 h-4 text-indigo-500" /><span className="text-[10px] font-black">{item.name}</span></div>
                          </th>
                        );
                      })}
                    </tr>
                    ...
                    {processedPermUsers.map((user) => {
                      ...
                      return (
                        <tr key={user.id} className="hover:bg-gray-50/80 transition-colors">
                          <td className="px-4 py-3 font-mono text-xs text-gray-600 border-r border-gray-200">
                          ...
                          <td className="px-4 py-3 border-r border-gray-200">
                            <div className="flex items-center gap-3">
                              <div className="w-8 h-8 rounded-full bg-gray-100 flex items-center justify-center overflow-hidden shrink-0 border border-gray-200">
                                {user.photoURL ? <img src={user.photoURL} alt="avatar" className="w-full h-full object-cover" /> : <Users className="w-4 h-4 text-gray-400" />}
                              </div>
                              <div className="flex flex-col min-w-0">
                                <span className="text-gray-900 font-bold text-sm truncate">{user.name}</span>
                                <div className="flex items-center gap-1 mt-1">
                                  {isAdmin ? <span className="text-[9px] text-rose-700 bg-rose-50 px-1.5 py-0.5 rounded border border-rose-200 truncate">システム管理者</span> : (user as any).positionName ? <span className="text-[9px] text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded border border-indigo-100 truncate">{(user as any).positionName}</span> : <span className="text-[9px] text-gray-400">一般</span>}
                                  {(user as any).isManager && <span className="text-[9px] text-emerald-600 bg-emerald-50 px-1.5 py-0.5 rounded border border-emerald-100 shrink-0">マネ</span>}
                                </div>
                              </div>
                            </div>
                          </td>
                          <td className="px-4 py-3 text-center border-r border-gray-200">
                            <button onClick={() => toggleCategoryAllForUser(user.id, !isAllCategoryAllowed, userPerms, isAdmin)} disabled={isProcessing || isAdmin} className={`px-3 py-1 rounded-lg text-[10px] font-bold transition-colors border ${isAdmin ? "bg-gray-100 text-gray-400 border-gray-200 opacity-50 cursor-not-allowed" : isAllCategoryAllowed ? "bg-indigo-50 text-indigo-700 border-indigo-200 hover:bg-indigo-100" : "bg-gray-50 text-gray-600 border-gray-200 hover:bg-gray-100"}`}>
                              {isProcessing ? <Loader2 className="w-3.5 h-3.5 animate-spin mx-auto" /> : (isAdmin ? "固定許可" : isAllCategoryAllowed ? "全解除" : "全許可")}
                            </button>
                          </td>
                          {EXTERNAL_PERMISSIONS.map(item => {
