...
export default function MessageHistory({ messages, tenants, users, setQueryParams, requestDelete }: Props) {
  ...
  const getTargetName = (msg: SystemMessage) => {
    ...
    if (msg.targetType === "user") {
      ...
      return msg.targetIds.map(uid => {
        ...
        const tName = tenants.find(t => t.id === u?.schoolId)?.name || "不明";
  ...
  const getTenantCompletion = (msg: SystemMessage, tenantId: string) => {
    ...
    if (total === 0) return { total: 0, responded: 0, isCompleted: true }; 
    const responded = tUsers.filter(u => msg.responses?.includes(u.id)).length;
    const isCompleted = msg.responseType === "single" ? responded > 0 : responded >= total;
    return { total, responded, isCompleted };
  };
  ...
  const processedMessages = useMemo(() => {
    let filtered = messages.filter(msg => {
      if (searchQuery) {
        ...
        if (!msg.title.toLowerCase().includes(query) && !msg.content.toLowerCase().includes(query) && !targetNames.includes(query)) {
          return false;
        }
      }
      ...
      if (tenantFilter !== "all") {
        if (msg.targetType === "tenant") {
          const tIds = msg.targetIds || (msg.targetId ? [msg.targetId] : []);
          if (!tIds.includes(tenantFilter)) return false;
        } else if (msg.targetType === "user" || msg.targetType === "department") {
          const tenantUserIds = users.filter(u => u.schoolId === tenantFilter).map(u => u.id);
          const targetUids = msg.targetIds || (msg.targetId ? [msg.targetId] : []);
          const hasTenantUser = targetUids.some(uid => tenantUserIds.includes(uid));
          if (!hasTenantUser) return false;
        }
      }
      ...
      const isCompleted = msg.requireResponse && (msg.responseType === "single" ? cCount === vCount && vCount > 0 : oResp >= oTotal && oTotal > 0);
    ...
    return filtered;
  }, [messages, searchQuery, categoryFilter, statusFilter, responseFilter, tenantFilter, sortBy, users, tenants]);
