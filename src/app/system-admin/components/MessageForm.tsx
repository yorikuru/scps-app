...
export default function MessageForm({ tenants, users, showAlert, editMessage, onSuccess, onCancel }: Props) {
  ...
  const [targetType, setTargetType] = useState<"all" | "tenant" | "department" | "user">("all");
  ...
  useEffect(() => {
    if (isEdit && editMessage) {
      ...
      setTargetType(editMessage.targetType);
      
      setTargetIds(editMessage.targetIds && editMessage.targetIds.length > 0 ? editMessage.targetIds : (editMessage.targetId ? [editMessage.targetId] : []));
  ...
  const filteredTenants = useMemo(() => {
    ...
    return tenants.filter(t => t.name.toLowerCase().includes(q) || t.schoolCode.toLowerCase().includes(q));
  }, [tenants, targetSearchQuery]);
  ...
  const getTenantName = (schoolId?: string) => {
    if (!schoolId) return "不明";
    const t = tenants.find(tenant => tenant.id === schoolId);
    return t ? t.name : "不明";
  };
  ...
  const executeSave = async () => {
    ...
    try {
      ...
      if (targetType === "user" || targetType === "tenant") finalTargetIds = targetIds;
      ...
      if (isEdit && editMessage) {
        ...
        batch.set(msgRef, {
          ...
          schoolId: "SYSTEM", senderId: "SYSTEM", senderName: "システム管理者", senderRole: "system_admin", senderSchoolId: "SYSTEM"
        });
      }
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (targetType === "tenant" && targetIds.length === 0) { showAlert("warning", "テナントを選択してください。"); return; }
    if (targetType === "user" && targetIds.length === 0) { showAlert("warning", "配信先のユーザーを選択してください。"); return; }
  ...
  return (
    <>
      <form onSubmit={handleSubmit} className="max-w-7xl mx-auto space-y-6 pb-12 p-6">
        {isEdit && (
          <div className="bg-blue-50 border border-blue-200 p-4 rounded-xl flex items-center justify-between shadow-sm animate-fade-in">
            <div className="flex items-center text-blue-800 font-bold text-sm">
              ...
              既存メッセージの編集モード
              <span className="ml-3 text-xs font-medium text-blue-600 border border-blue-200 bg-white px-2 py-0.5 rounded-full">保存すると全ユーザーの画面に再表示（未読化）されます</span>
            </div>
            <button type="button" onClick={onCancel} className="text-xs font-bold text-gray-500 hover:text-gray-800 bg-white px-4 py-2 rounded-lg border border-gray-200 transition-colors shadow-xs">
              編集をキャンセル
            </button>
          </div>
        )}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
          <div className="lg:col-span-8 space-y-6">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 space-y-6">
              <div className="border-b border-gray-100 pb-3 flex items-center">
                ...
                <h4 className="text-base font-black text-gray-900">メッセージ内容</h4>
              </div>

              <div>
                <label className="block text-sm font-bold text-gray-700 mb-1.5">タイトル <span className="text-red-500">*</span></label>
                <input type="text" required value={title} onChange={e => setTitle(e.target.value)} className="w-full border border-gray-300 rounded-xl py-3 px-4 text-sm font-medium focus:ring-2 focus:ring-blue-500 outline-none transition-all shadow-xs" placeholder="例: システムメンテナンスのお知らせ" />
              </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 space-y-4">
              <div className="border-b border-gray-100 pb-3 flex items-center justify-between">
                <div className="flex items-center">
                  ...
                  <h4 className="text-base font-black text-gray-900">添付ファイル</h4>
                </div>
                <span className="text-xs font-bold text-gray-500 bg-gray-50 px-2 py-1 rounded-lg border border-gray-200">{existingAttachments.length + newAttachments.length} / 3</span>
              </div>

              <div className="space-y-3">
                {existingAttachments.map((att, i) => (
                  <div key={`old-${i}`} className="flex items-center justify-between p-2.5 bg-gray-50 border border-gray-200 rounded-xl">
                ...
                {newAttachments.map((file, i) => (
                  <div key={`new-${i}`} className="flex items-center justify-between p-2.5 bg-blue-50 border border-blue-200 rounded-xl">
                    <div className="flex items-center gap-2 overflow-hidden pr-2">
                    ...
                    }} className="p-1.5 text-blue-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors flex-shrink-0">
                      <X className="w-4 h-4" />
                    </button>
                  </div>
                ))}
                ...
                {existingAttachments.length + newAttachments.length < 3 && (
                  <label className="flex items-center justify-center p-5 border-2 border-dashed border-gray-300 rounded-xl cursor-pointer hover:bg-gray-50 hover:border-blue-400 transition-colors">
                    ...
                    <span className="text-xs font-bold text-gray-500 flex items-center">
                      <Paperclip className="w-4 h-4 mr-1.5" /> ファイルを選択して追加 (最大3ファイル)
                    </span>
                  </label>
                )}
              </div>
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-200 space-y-5">
              <div className="border-b border-gray-100 pb-3 flex items-center">
                <div className="p-1.5 bg-indigo-50 rounded-lg mr-2"><Globe className="w-4 h-4 text-indigo-600" /></div>
                <h4 className="text-base font-black text-gray-900">配信ターゲット（宛先）</h4>
              </div>

              <div className="flex flex-wrap gap-3 p-1.5 bg-gray-50 rounded-xl border border-gray-200 w-fit">
                <label className={`flex items-center px-4 py-2 rounded-lg cursor-pointer transition-colors ${targetType === "all" ? "bg-white shadow-sm border border-gray-200 font-bold text-blue-700" : "text-gray-500 hover:text-gray-700 font-medium"}`}>
                  <input type="radio" checked={targetType === "all"} onChange={() => { setTargetType("all"); setTargetIds([]); setTargetDepartments([]); }} className="hidden" />
                  <Globe className={`w-4 h-4 mr-2 ${targetType === "all" ? "text-blue-600" : "opacity-50"}`} /> SCPS一斉
                </label>
                <label className={`flex items-center px-4 py-2 rounded-lg cursor-pointer transition-colors ${targetType === "tenant" ? "bg-white shadow-sm border border-gray-200 font-bold text-blue-700" : "text-gray-500 hover:text-gray-700 font-medium"}`}>
                  ...
                  <Building2 className={`w-4 h-4 mr-2 ${targetType === "tenant" ? "text-blue-600" : "opacity-50"}`} /> テナントを指定
                </label>
                <label className={`flex items-center px-4 py-2 rounded-lg cursor-pointer transition-colors ${targetType === "department" ? "bg-white shadow-sm border border-gray-200 font-bold text-blue-700" : "text-gray-500 hover:text-gray-700 font-medium"}`}>
                  <input type="radio" checked={targetType === "department"} onChange={() => { setTargetType("department"); setTargetIds([]); }} className="hidden" />
                  <Network className={`w-4 h-4 mr-2 ${targetType === "department" ? "text-blue-600" : "opacity-50"}`} /> 部門・グループ
                </label>
                <label className={`flex items-center px-4 py-2 rounded-lg cursor-pointer transition-colors ${targetType === "user" ? "bg-white shadow-sm border border-gray-200 font-bold text-blue-700" : "text-gray-500 hover:text-gray-700 font-medium"}`}>
                  <input type="radio" checked={targetType === "user"} onChange={() => { setTargetType("user"); setTargetDepartments([]); }} className="hidden" />
                  <Users className={`w-4 h-4 mr-2 ${targetType === "user" ? "text-blue-600" : "opacity-50"}`} /> 個人を選択
                </label>
              </div>

              {targetType === "tenant" && (
                <div className="border border-gray-200 rounded-xl overflow-hidden bg-gray-50/50 animate-fade-in">
                  <div className="p-3 border-b border-gray-200 bg-white flex items-center justify-between">
                    <div className="relative w-full max-w-md">
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                      <input type="text" placeholder="テナント名や学校コードで検索..." value={targetSearchQuery} onChange={e => setTargetSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2 text-xs font-bold border border-gray-300 rounded-lg focus:ring-blue-500 outline-none" />
                    </div>
                    <div className="text-xs font-bold text-indigo-700 bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 whitespace-nowrap">選択中: {targetIds.length}組織</div>
                  </div>
                  <div className="max-h-[250px] overflow-y-auto p-2 bg-white custom-scrollbar grid grid-cols-1 md:grid-cols-2 gap-2">
                    {filteredTenants.map(t => (
                      <label key={t.id} className={`flex items-center p-2.5 rounded-xl cursor-pointer transition-all border ${targetIds.includes(t.id) ? 'bg-blue-50 border-blue-200 shadow-sm' : 'hover:bg-gray-50 border-transparent'}`}>
                        <input type="checkbox" checked={targetIds.includes(t.id)} onChange={(e) => setTargetIds(e.target.checked ? [...targetIds, t.id] : targetIds.filter(id => id !== t.id))} className="h-4 w-4 text-blue-600 rounded mr-3 flex-shrink-0" />
                        <span className="text-sm text-gray-900 font-bold truncate">{t.name} <span className="text-gray-500 font-normal">({t.schoolCode})</span></span>
                      </label>
                    ))}
                  </div>
                </div>
              ...
              {targetType === "department" && (
                <div className="p-4 bg-gray-50 rounded-xl border border-gray-200 animate-fade-in space-y-4">
                  <div className="flex justify-between items-center pb-2 border-b border-gray-200">
                    <span className="text-xs font-bold text-gray-700">対象グループを選択（複数可）</span>
                    <span className="text-xs font-bold text-indigo-700 bg-indigo-50 px-2 py-1 rounded border border-indigo-100">該当: {departmentUserIds.length}名</span>
                  </div>
                  <div className="space-y-3">
                    <div className="flex flex-wrap gap-2">
                      <label className="flex items-center p-2 bg-white border border-gray-200 rounded-lg cursor-pointer hover:bg-blue-50 transition-colors">
                        <input type="checkbox" checked={targetDepartments.includes("manager")} onChange={() => toggleDepartment("manager")} className="w-4 h-4 text-blue-600 mr-2 rounded" />
                        <span className="text-xs font-bold">マネージャー権限層</span>
                      </label>
                      <label className="flex items-center p-2 bg-white border border-gray-200 rounded-lg cursor-pointer hover:bg-blue-50 transition-colors">
                        <input type="checkbox" checked={targetDepartments.includes("role_teacher")} onChange={() => toggleDepartment("role_teacher")} className="w-4 h-4 text-blue-600 mr-2 rounded" />
                        <span className="text-xs font-bold">教職員</span>
                      </label>
                      <label className="flex items-center p-2 bg-white border border-gray-200 rounded-lg cursor-pointer hover:bg-blue-50 transition-colors">
                        <input type="checkbox" checked={targetDepartments.includes("role_officer")} onChange={() => toggleDepartment("role_officer")} className="w-4 h-4 text-blue-600 mr-2 rounded" />
                        <span className="text-xs font-bold">生徒会役員</span>
                      </label>
                      <label className="flex items-center p-2 bg-white border border-gray-200 rounded-lg cursor-pointer hover:bg-blue-50 transition-colors">
                        <input type="checkbox" checked={targetDepartments.includes("role_student")} onChange={() => toggleDepartment("role_student")} className="w-4 h-4 text-blue-600 mr-2 rounded" />
                        <span className="text-xs font-bold">一般生徒</span>
                      </label>
                    </div>
                    {availablePositions.length > 0 && (
                      <div className="pt-3 border-t border-gray-200/60">
                        <span className="text-[10px] font-bold text-gray-400 block mb-2">役職別フィルター</span>
                        <div className="flex flex-wrap gap-2">
                  ...
                  {departmentUserIds.length > 0 && (
                    <div className="pt-3 border-t border-gray-200">
                      <span className="text-[10px] font-bold text-gray-500 block mb-1.5">配信対象者一覧</span>
                      <div className="flex flex-wrap gap-1.5 max-h-32 overflow-y-auto custom-scrollbar p-2 bg-white border border-gray-200 rounded-lg">
                        {departmentUserIds.map(uid => {
                          const u = users.find(user => user.id === uid);
                          return <span key={uid} className="text-[10px] font-bold text-gray-700 bg-gray-100 px-1.5 py-0.5 rounded border border-gray-200">{u?.name || "不明"} <span className="font-normal opacity-70">({getTenantName(u?.schoolId)})</span></span>;
                        })}
                      </div>
                    </div>
                  )}
                ...
                <div className="border border-gray-200 rounded-xl overflow-hidden bg-gray-50/50 animate-fade-in">
                  <div className="p-3 border-b border-gray-200 bg-white flex justify-between items-center">
                    <div className="relative flex-1 max-w-md">
                      <Search className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-gray-400" />
                      <input type="text" placeholder="名前やIDで検索..." value={targetSearchQuery} onChange={e => setTargetSearchQuery(e.target.value)} className="w-full pl-9 pr-3 py-2 text-xs font-bold border border-gray-300 rounded-lg focus:ring-blue-500 outline-none" />
                    </div>
                    <div className="text-xs font-bold text-indigo-700 bg-indigo-50 px-3 py-1.5 rounded-lg border border-indigo-100 whitespace-nowrap">選択中: {targetIds.length}名</div>
                  </div>
                  <div className="max-h-[250px] overflow-y-auto p-2 bg-white custom-scrollbar grid grid-cols-1 md:grid-cols-2 gap-2">
                    {filteredUsers.map(u => (
                      <label key={u.id} className={`flex items-center p-2.5 rounded-xl cursor-pointer transition-all border ${targetIds.includes(u.id) ? 'bg-blue-50 border-blue-200 shadow-sm' : 'hover:bg-gray-50 border-transparent'}`}>
                        <input type="checkbox" checked={targetIds.includes(u.id)} onChange={e => setTargetIds(e.target.checked ? [...targetIds, u.id] : targetIds.filter(id => id !== u.id))} className="h-4 w-4 text-blue-600 rounded mr-3 flex-shrink-0" />
                        <div className="flex flex-col min-w-0">
                          <span className="text-sm text-gray-900 font-bold truncate">{u.name} <span className="text-[10px] text-gray-500 font-normal">({getTenantName(u.schoolId)})</span></span>
                          <span className="text-[10px] text-gray-500 truncate">{u.email || "メール未設定"}</span>
                        </div>
                      </label>
                    ))}
                  </div>
          ...
          <div className="lg:col-span-4 space-y-6 sticky top-6">
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 space-y-5">
              ...
              <div>
                <label className="block text-xs font-bold text-gray-500 mb-2 uppercase tracking-wider">カテゴリ <span className="text-red-500">*</span></label>
                <div className="grid grid-cols-2 gap-2">
                  {Object.entries(CATEGORIES).map(([cat, info]) => (
                    <label key={cat} className={`cursor-pointer flex items-center justify-center py-2.5 px-1 rounded-xl border-2 transition-all ${category === cat ? `${info.color} border-current bg-white shadow-sm` : 'border-gray-100 bg-gray-50 hover:bg-gray-100 text-gray-500'}`}>
              ...
              <div className="pt-2 border-t border-gray-100">
                <label className="block text-xs font-bold text-gray-500 mb-2 uppercase tracking-wider">追加バッジ (任意)</label>
                <div className="flex flex-wrap gap-2">
                  <label className={`cursor-pointer px-3 py-1.5 rounded-lg text-xs font-bold border transition-colors ${subBadge === "none" ? "bg-gray-800 text-white border-gray-800" : "bg-white text-gray-500 border-gray-200 hover:bg-gray-50"}`}>
                    <input type="radio" className="hidden" checked={subBadge === "none"} onChange={() => setSubBadge("none")} /> なし
                  </label>
                  <label className={`cursor-pointer px-3 py-1.5 rounded-lg text-xs font-bold border transition-colors ${subBadge === "update1" ? "bg-yellow-100 text-yellow-800 border-yellow-300" : "bg-white text-yellow-700 border-yellow-200 hover:bg-yellow-50"}`}>
                    <input type="radio" className="hidden" checked={subBadge === "update1"} onChange={() => setSubBadge("update1")} /> 更新①
                  </label>
                  <label className={`cursor-pointer px-3 py-1.5 rounded-lg text-xs font-bold border transition-colors ${subBadge === "update2" ? "bg-orange-100 text-orange-800 border-orange-300" : "bg-white text-orange-700 border-orange-200 hover:bg-orange-50"}`}>
            ...
            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 space-y-4">
              ...
              <div>
                <label className="block text-[10px] font-bold text-gray-500 mb-1">掲載開始日時 (任意)</label>
                <input type="datetime-local" value={startAt} onChange={e => setStartAt(e.target.value)} className="w-full border border-gray-300 rounded-xl py-2 px-3 text-xs font-medium focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>
              <div>
                <label className="block text-[10px] font-bold text-gray-500 mb-1">掲載終了日時 (任意)</label>
                <input type="datetime-local" value={endAt} onChange={e => setEndAt(e.target.value)} className="w-full border border-gray-300 rounded-xl py-2 px-3 text-xs font-medium focus:ring-2 focus:ring-blue-500 outline-none" />
              </div>
            </div>

            <div className="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 space-y-3">
              <h4 className="text-sm font-black text-gray-900 flex items-center border-b border-gray-100 pb-2 mb-3"><Settings2 className="w-4 h-4 mr-1.5 text-gray-400" /> オプション設定</h4>

              <label className={`flex items-start p-3 border rounded-xl cursor-pointer transition-all ${isImportant ? 'bg-red-50 border-red-200' : 'bg-white border-gray-200 hover:bg-gray-50'}`}>
                <div className="flex items-center h-5"><input type="checkbox" checked={isImportant} onChange={e => setIsImportant(e.target.checked)} className="h-4 w-4 text-red-600 rounded border-gray-300" /></div>
                <div className="ml-3">
                  <span className={`block text-xs font-bold flex items-center ${isImportant ? 'text-red-800' : 'text-gray-700'}`}><Pin className="w-3 h-3 mr-1" /> 緊急として上部に固定</span>
                  <span className="block text-[9px] text-gray-500 mt-1 leading-tight">ダッシュボードの最上部に赤いバッジ付きで固定表示されます。</span>
                </div>
              </label>

              <div className={`p-3 border rounded-xl transition-all ${requireResponse ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-200 hover:bg-gray-50'}`}>
                <label className="flex items-start cursor-pointer">
                  <div className="flex items-center h-5"><input type="checkbox" checked={requireResponse} onChange={e => setRequireResponse(e.target.checked)} className="h-4 w-4 text-blue-600 rounded border-gray-300" /></div>
                  <div className="ml-3">
                    <span className={`block text-xs font-bold flex items-center ${requireResponse ? 'text-blue-800' : 'text-gray-700'}`}><CheckSquare className="w-3 h-3 mr-1" /> 対応応答を要求する</span>
                    <span className="block text-[9px] text-gray-500 mt-1 leading-tight">ユーザーに「対応済みにする」ボタンを表示します。</span>
                  </div>
                </label>
                {requireResponse && (
                  <div className="mt-3 ml-7 pt-3 border-t border-blue-100/50 flex flex-col gap-2">
                    <label className="flex items-center cursor-pointer">
                      <input type="radio" checked={responseType === "single"} onChange={() => setResponseType("single")} className="h-3.5 w-3.5 text-blue-600" />
                      <span className="ml-2 text-[10px] font-bold text-gray-700">誰か1人が対応すれば完了</span>
                    </label>
                    <label className="flex items-center cursor-pointer">
                      <input type="radio" checked={responseType === "all"} onChange={() => setResponseType("all")} className="h-3.5 w-3.5 text-blue-600" />
                      <span className="ml-2 text-[10px] font-bold text-gray-700">全員が対応必須</span>
                    </label>
                  </div>
                )}
              </div>

              <label className={`flex items-start p-3 border rounded-xl cursor-pointer transition-all ${isDismissible ? 'bg-gray-50 border-gray-200' : 'bg-white border-gray-200 hover:bg-gray-50'}`}>
                <div className="flex items-center h-5"><input type="checkbox" checked={isDismissible} onChange={e => setIsDismissible(e.target.checked)} className="h-4 w-4 text-indigo-600 rounded border-gray-300" /></div>
                <div className="ml-3">
                  <span className="block text-xs font-bold text-gray-700 flex items-center"><EyeOff className="w-3 h-3 mr-1" /> ユーザーの非表示(既読)を許可</span>
                </div>
              </label>

              <label className={`flex items-start p-3 border rounded-xl cursor-pointer transition-all ${showSenderName ? 'bg-gray-50 border-gray-200' : 'bg-white border-gray-200 hover:bg-gray-50'}`}>
                <div className="flex items-center h-5"><input type="checkbox" checked={showSenderName} onChange={e => setShowSenderName(e.target.checked)} className="h-4 w-4 text-indigo-600 rounded border-gray-300" /></div>
                <div className="ml-3">
      ...
      {confirmResetDialog && (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-black/50 backdrop-blur-sm px-4 py-6 animate-fade-in">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-sm overflow-hidden p-6 text-center border border-gray-100">
            ...
            <p className="text-xs font-bold text-gray-600 mb-5 leading-relaxed whitespace-pre-wrap">
